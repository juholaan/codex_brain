...
#   ~/.codex/secret-warn/   (skill files + audit log)
#   ~/.codex/settings.json  (hook registration, non-destructive merge)

set -euo pipefail

SKILL_SRC="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SKILL_DEST="${SECRET_WARN_ROOT:-$HOME/.codex/secret-warn}"
...
# Merge hook entries into ~/.codex/settings.json (non-destructive)
python3 - <<PYMERGE
import json, os
