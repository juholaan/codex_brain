...
- **Auto-triggered (fast mode, optional):** When logging a decision with `stakes: high` in any decision template, Codex can auto-offer: "This is high-stakes. Want me to deconstruct it before you commit?" If yes, run fast mode (Phase 1 + Phase 4 only). To enable, add `deconstruct_auto: true` to your decision template frontmatter.
