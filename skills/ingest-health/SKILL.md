...
Loads Apple Health data into `~/.codex/health-mcp/health.duckdb` so the rest of the substrate (daily-journal, coaching, advisory-panel, patterns, insights) can read biometrics through health-mcp tools.
...
1. **Storage stays local.** The DuckDB file lives at `~/.codex/health-mcp/health.duckdb`. Treat it as a medical record.
