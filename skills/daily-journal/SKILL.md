...
**Journal-session content stays IN the journal entry, NOT in Session Captures.** Session Captures is for verbatim quotes from OTHER Codex sessions throughout the day — content that exists outside the journal interview. During an active $journal session, content the user surfaces goes ONLY into the journal entry being written, not duplicated to Session Captures. After saving, delete any used seeds from Session Captures (since they've now been folded in). Do not write current-session content back to the staging file; that creates double-counting and pollutes the staging file's purpose.
...
**0b. Session Captures** (gated on `data_sources.session_captures: on`). Read `⚙️ Meta/Session Captures.md` (or your vault's equivalent staging file) in full. This file contains verbatim quotes the user said throughout the day across all their Codex sessions, things they've likely forgotten by the time they journal.
...
**Why some sources are opt-in:** iMessage, WhatsApp, and Calendar see private conversations and meetings. The user should consent explicitly per vault, not by default. The other three (RescueTime, Session Captures, Today's activity) read your own data and are on by default. Codified 2026-05-08 to close two gaps: warm Codex sessions don't fire their close cascade in real time, and most relational events happen in iMessage/WhatsApp/in-person rather than inside a Codex session. The journal must see the whole day, but the user opts in per vault to the cross-platform pulls.
...
Capture their answer. After saving the journal entry (Step 9), update the weekly focus file (check AGENTS.md for the path, e.g. `🏠 Home/✅ This Week.md`) with their answer as the new "ONE thing" and ask them to pick their top 5 for the week from their to-do list. Replace the previous week's items. This is the weekly reset for the focusing file.
...
- If 2+ blocks inferred: "That's [X] blocks per RescueTime. The chain continues." Mark today in the Deep Work Chain file (check AGENTS.md for the path, e.g. `Home/Deep Work Chain.md`).
...
**Always use Bash (`cat`) to read and write journal files — do NOT use the Read tool.** The Read tool fails silently on emoji folder paths in worktree sessions (a known Codex limitation). Use:
...
If the vault's `AGENTS.md` states a filename rule, it wins — `filename_format` should already match it; if they ever disagree, follow `AGENTS.md`.
...
1. Save them to the Idea Quarantine file (check AGENTS.md for the path, e.g. `💼 Business/Idea Quarantine.md`) under the `## Ideas` section
...
If you find any:
1. Read the personal to-do file (check AGENTS.md for the path) to check for duplicates
...
If the Step 5 panel surfaced a dissent or an omission flag, automatically append it to the Panel Feedback Log (check AGENTS.md for the path, e.g. `🏠 Home/Panel Feedback Log.md`) under the synthetic panel reactions section. Use this format:
...
```bash
TOKEN_FILE="$HOME/.codex/.ai-brain-starter-email-on-file"
SENTINEL="$HOME/.codex/.ai-brain-starter-first-journal-fired"
...
If `$HOME/.codex/.ai-brain-starter-email-on-file` does not exist, or holds `declined` / `recorded` rather than a hex token, skip the call entirely. The user has not opted into the email loop, so we have no token to attach. Never ask for the email here — journaling is not a capture surface.
