...
Check whether the two automation hooks are wired in `~/.codex/hooks.json`:
...
Red if expected hooks not in settings.json. Yellow if the hook script files exist but aren't registered. Green if both registered AND firing recently (check `~/.codex/hookify-blocks.log` for the hook name in the last 48h).
...
Plus:
- Read `~/.codex/hooks.json` to verify hooks are registered
- Read `~/.codex/hookify-blocks.log` to verify hooks have fired recently
