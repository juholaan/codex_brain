---
name: "setup-brain"
description: "Set up or upgrade an AI-powered Obsidian vault. Interviews you, builds your vault structure (or works with what you already have), creates your AGENTS.md memory file, installs tools, and gets you journaling — all in one conversation. Also has a repair/upgrade path for existing users."
---
...
| "Quit and reopen Codex, then paste..." / "hooks load properly in a clean session" | NEVER. Continue in this session. Hooks already loaded. |
| "Leave this session — `cd ~/plugins/codex-brain-starter && codex`, then run `$setup-brain`" / bouncing the user to a NEW session (or any `cd` / manual `$setup-brain`) to begin the interview | NEVER. Continue Phase 1 HERE. The ONE exception is a surface that genuinely cannot run commands (web chat, read-only, no tools): then route the user to the **Codex desktop app** + the install prompt from the README's Install section, which re-enters this flow. They never type `$setup-brain` or `cd`. |
...
**Telemetry capture.** After each phase completes, append one JSON line to `~/.codex/.ai-brain-starter-install.jsonl` describing what happened. Schema:
...
**Version check:** Before starting, read `.codex-plugin/plugin.json` and report the installed version only if the user asks. Marketplace installs may be copied snapshots with no `.git` directory, so never assume `git pull` is a valid updater. Updates happen through the Plugins Directory or by rerunning `scripts/install_codex_plugin.py` from a newer checkout. After an update, read `docs/CHANGELOG.md` and summarize relevant changes in plain English.

## Already Set Up? Use This Instead

If they've already run setup and are coming back to fix or upgrade something, ask: "Are you looking to (1) add a new feature like book notes or a team vault, (2) fix something that's broken, or (3) upgrade your AGENTS.md with the latest improvements?"

- **Add a feature:** Jump to the relevant phase. Book notes → Phase 12. Team vault → Phase 20. Don't re-run the full setup.
- **Fix something broken:** Run `$diagnose` first — it checks 10 common failure points (AGENTS.md, hooks, journal index, .ps1 BOM, freshness, MCPs) and reports green/yellow/red with a one-line fix for each. If `$diagnose` doesn't surface the issue, ask what's wrong. Common issues:
  - Vault map empty → open their AGENTS.md and fill in the `## Vault Map` section
  - Journal skill not saving → check `~/.agents/skills/daily-journal/SKILL.md` exists
  - Insights not finding entries → check `⚙️ Meta/journal-index.json` exists; if not, re-run index generation from Phase 18
  - Codex creating duplicate folders → vault map is missing or wrong; fix it first
  - Windows PowerShell parser errors → re-run `bootstrap.ps1` after `git pull` (BOM fix shipped 2026-04-22)
- **Upgrade AGENTS.md:** Read their existing AGENTS.md. Compare to the Phase 4 template. Add missing sections without overwriting personal content. Never replace, only add.

---

## Modular Phase Architecture

This setup has 25 phases (0-24). Each phase is stored in its own file under `phases/`. **Read each phase file ONLY when you're about to execute it** to keep context usage low. Large embedded templates (AGENTS.md template, insights skill, etc.) are in `templates/generated/` and referenced by the phase files.
...
| 4 | `phases/phase-04-agents-md.md` | Build their AGENTS.md (interview + template). Template at `templates/generated/agents-md-template.md` |
...
After each phase completes, write the phase number + an ISO timestamp to `~/.codex/.ai-brain-starter-progress.json`:
...
1. **Check progress file.** Read `~/.codex/.ai-brain-starter-progress.json`. If present, jump to the first un-completed phase. If absent, start at Phase 0.
2. Read the phase file for the current phase.
3. Execute it (interview the user, create files, install tools).
4. When the phase completes, append a telemetry line to `~/.codex/.ai-brain-starter-install.jsonl` AND update the progress file (`last_completed_phase`). Helper script: `scripts/install-telemetry.py append <phase> <outcome>` (falls back to `jq`-based bash if Python isn't ready).
...
- **NEVER ask the user to open a terminal during setup.** Codex runs all bash commands via its own tools. Users should not see a terminal after bootstrap runs. If something needs a shell command, Codex does it — it never says "open terminal and run X."
...
| Codex permission prompt | ES: "Cuadro pide permiso. Dale 'Allow'. Soy yo." · EN: "Box asks permission. Click 'Allow'. That's me." |
| **⌘↩ vs typing — this is the most common point of confusion** | When Codex is waiting to run a tool (gray box with a tool name), press **⌘↩** (Mac) or **Ctrl↩** (Windows). When Codex asks you a question and is waiting for YOUR answer, just **type normally and press Enter**. Rule: if you see a gray tool box → ⌘↩. If Codex ends with a question mark → type your answer. ES: "Si ves una caja gris con un nombre de herramienta → ⌘↩. Si Codex te hace una pregunta → escribe tu respuesta normal y enter." |
