...
**If a Time Tracking file exists** (check AGENTS.md for the path, typically `⚙️ Meta/Time Tracking.md`): Read it and filter entries for the period. This shows what categories were worked on during Codex sessions (Writing, Business, Vault, Personal, Admin). Merge with RescueTime app data for a combined picture: RescueTime shows which apps were used, session logs show what purpose they served.
...
If the vault has `scripts/token-usage-report.py`, run it to surface real per-session token consumption + estimated USD cost from Codex session JSONLs:
...
This replaces guess-work with real numbers from `~/.codex/projects/<vault-hash>/*.jsonl` (the only ground-truth source for token consumption — Codex does not surface this in any UI). Inspired by JuliusBrussee/caveman's /caveman-stats.
