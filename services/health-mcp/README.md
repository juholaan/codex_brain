...
32 tools across 8 categories. Three Apple Health ingestion modes plus manual lab CSV import. Open scoring algorithms with weights documented inline. DuckDB storage at `~/.codex/health-mcp/health.duckdb`.
...
Restart Codex. `codex mcp list` should show `health: ✓ Connected`.
...
- Local-only by design. DuckDB at `~/.codex/health-mcp/health.duckdb`. No cloud sync, no telemetry.
