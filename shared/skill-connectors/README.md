...
The two-line `sys.path` shim works whether the skill is run as `python3 skills/<name>/ingest.py` from repo root or installed into `~/.agents/skills/<name>/` (the symlinked end-user location).
