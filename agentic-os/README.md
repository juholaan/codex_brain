...
|---|---|---|
| **Kernel** | `kernel/AGENTS.md` | A small COO/orchestrator. Routes work to agents in a plain markdown table. Stays under 100 lines — detail lives in the files it points to. |
...
Lands `AGENTS.md` (kernel) + `.claude/agents/` + `.claude/contexts/` +
`.claude/rules/<lang>/` + `.codex/hooks/` + a `data/` memory scaffold, plus a
