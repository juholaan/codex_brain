...
4. **Humanize external-facing prose before it leaves your hands.** Any prose you write for a human audience — a client email, a LinkedIn post, a Substack draft, a pitch doc, a newsletter, an essay — gets `/humanizer` run on it before it's considered done. The skill strips the AI-isms that give you away. Don't ask, just run it. **Scope:** prose only. Skip YAML, code, tables, dashboards, runbooks, meta files, journal entries, and single-line edits. For non-trivial changes to a humanized doc, re-run on the section you touched, not the whole file. The humanizer skill was installed in Phase 0 — if it's missing, re-run `git clone https://github.com/adelaidasofia/humanizer.git ~/.agents/skills/humanizer`.
...
8. **Your brain lives in the vault — never only in Codex's project memory.** Codex's own memory dir (`~/.codex/projects/<vault>/memory/`) is wired during setup to be a symlink into this vault at `⚙️ Meta/Agent Memory/`, so what Codex remembers is a real file in your vault, visible in Obsidian and saved in your vault's history. Two rules follow:
   - **Dual-write durability.** Anything worth remembering across sessions MUST end up as a vault file (a memory file in `⚙️ Meta/Agent Memory/`, a note, a decision, an updated rule). Never leave a durable fact ONLY in project memory — it is tied to one machine and one tool. Test: "Different computer, different tool, next month — is this still here?" If the answer needs Codex specifically, it is in the wrong place.
   - **Cross-project routing (when running Codex from another repo).** If this session is running from a work/code project that is NOT the brain vault, personal or life content (journal-worthy reflections, people, decisions about your life, anything that belongs in the second brain) routes to the brain vault — **write it there, do not just offer.** Resolve the brain vault path from its AGENTS.md `# Memory` block or ask once if genuinely unknown, then save. Work-specific project notes stay with the project; only brain-bound content crosses over. The failure to avoid: saving personal content into a work repo's tool-memory dir, where it never reaches the brain.
...
3. During: Before giving strategic advice or analysis on topics where the vault graph is relevant, confirm you have read or queried the relevant graph this session. Memory and AGENTS.md are fast-recall indexes, not substitutes for graph depth on substantive questions. If new concepts come up, create notes in the right folder. For decisions, create per-decision files in `⚙️ Meta/Decisions/`. If the session exceeds 30 back-and-forth exchanges with the original task still open, flag it: "This session is running long. Want to compress and continue, or wrap up fresh?"
...
To recover from a partial close (model bailed + no API key was set), run `python3 ~/plugins/codex-brain-starter/scripts/recover-last-close.py`. To rollback the most recent close, `python3 ~/plugins/codex-brain-starter/scripts/undo-last-close.py`.
