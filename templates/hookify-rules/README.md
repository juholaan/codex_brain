...
1. Install hookify: In Codex, run `/plugin install hookify` or clone from the [claude-code repo](https://github.com/anthropics/claude-plugins-official/tree/main/plugins/hookify)
...
subset into `~/.codex/` on every install, so those rules fire on a fresh machine
with no manual step.
...
copy is copy-if-absent, so if you have already customized a rule in `~/.codex/`,
