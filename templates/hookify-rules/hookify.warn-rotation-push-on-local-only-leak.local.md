...
> rules never fire there: Codex sends the submitted text under the payload key
...
- Local machine only (session transcripts under `~/.codex/projects/...`, local logs, files on disk that never sync)
