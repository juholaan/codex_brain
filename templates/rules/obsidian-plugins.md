...
Three community plugins that extend what Codex can do with the vault. All configuration is on the Codex side (hooks, scripts, rules). You just install the plugins and leave them running.
...
**What it gives Codex**: The ability to open notes in Obsidian's UI, trigger Obsidian commands, and search via Dataview queries, all over HTTP.

### How Codex uses it

After creating or editing a vault file, Codex can open it in Obsidian automatically:
```bash
...
The API runs on `https://127.0.0.1:27124/` (localhost only, self-signed HTTPS). Codex uses `-k` flag with curl to accept the self-signed cert.
...
**What it gives Codex**: Semantic search (meaning-based, not just keyword/link-based) over the vault.
