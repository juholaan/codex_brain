...
- Registration goes in vault `.codex/config.toml`, NOT `~/.codex/.codex/config.toml` (ghost path)
...
7. **Register via edit, not duplicate.** If the MCP is already in vault `.codex/config.toml`, leave the entry; just restart Codex to pick up new tools.
...
Codex allows multiple sessions to touch the same directory. If you are in a worktree and another session (or agent) is operating on the same MCP:
...
- Confirm `.codex/config.toml` parses: `python3 -c "import json; json.load(open('<path>'))"`.
