...
**Mac / Linux:**
```bash
bash ~/plugins/codex-brain-starter/scripts/update-check.sh
```

**Windows (PowerShell):**
```powershell
powershell -File "$env:USERPROFILE\plugins\codex-brain-starter\scripts\update-check.ps1"
```
...
**Mac / Linux:**
```bash
bash ~/plugins/codex-brain-starter/bootstrap.sh
```

**Windows (PowerShell):**
```powershell
powershell -File "$env:USERPROFILE\plugins\codex-brain-starter\bootstrap.ps1"
```
...
You need the user's vault path to check vault-scope drift. The vault path is the directory that contains the `AGENTS.md` file you loaded at session start — walk up from the current working directory until you find a `AGENTS.md`, that ancestor directory is `$VAULT_PATH`. (If you can't find one, run drift-check without `--vault` and it'll only check the installed-skills scope.)

**Mac / Linux:**
```bash
bash ~/plugins/codex-brain-starter/scripts/drift-check.sh --vault "[VAULT_PATH]"
```

**Windows (PowerShell):**
```powershell
powershell -File "$env:USERPROFILE\plugins\codex-brain-starter\scripts\drift-check.ps1" -Vault "[VAULT_PATH]"
```
...
Scopes:
- `skill` — file under `~/.agents/skills/<skill>/` differs from `<starter>/skills/<skill>/<rel-path>`
- `vault-script` — file under `$VAULT/⚙️ Meta/scripts/<basename>` differs from `<starter>/scripts/<basename>`
- `vault-rule` — block in `$VAULT/AGENTS.md` (identified by an H1 heading) differs from `<starter>/templates/rules/<file>.md`
...
3. **For `vault-rule` drift**, the diff is between two markdown blocks. Be extra clear that the change is to a *block inside* their AGENTS.md, not the whole file. Say: "This is a block inside your vault's AGENTS.md, identified by the heading `[heading]`. I'd replace just that block, not the whole file."
...
6. **If the user says update** (or "update all"), do this in order — backup ALWAYS happens first, no exceptions:
   ...
   - **For `vault-rule` scope**: use the Edit tool with `old_string` = the entire installed block (heading line through last non-blank line before the next H1 or EOF) and `new_string` = the entire repo template content. Do NOT overwrite the whole AGENTS.md file — only replace the block.
   - Confirm to the user: "Updated. Backup at `<installed_path>.bak-...`."

7. **If the user says skip**, log it: "Skipped [basename]." Move on.

7a. **If the user says skip permanently**, append the installed path to `~/.codex/.ai-brain-starter-drift-check-ignore` (creating the file if it doesn't exist), then log: "Won't ask about [basename] again. To re-enable, edit `~/.codex/.ai-brain-starter-drift-check-ignore` and remove the line." Use this exact append idiom (Mac/Linux):
   ```bash
   echo "<installed_path>" >> "$HOME/.codex/.ai-brain-starter-drift-check-ignore"
   ```
   Or on Windows (PowerShell):
   ```powershell
   Add-Content -LiteralPath "$env:USERPROFILE\.codex\.ai-brain-starter-drift-check-ignore" -Value "<installed_path>"
   ```
...
- **For `vault-rule` drift, only replace the block — never the whole file.** AGENTS.md contains many concatenated rules and user-added context; touching anything outside the targeted block would silently nuke unrelated content.
...
If `~/plugins/codex-brain-starter/scripts/drift-check.sh` doesn't exist, the user is on a version that predates drift detection. Don't surface this — the next bootstrap will install it. Just skip the drift block silently and continue.
