...
For these cases, write a companion PreToolUse Python hook in `~/.codex/hooks/` and register it in `settings.json`. The hook reads `tool_input` from stdin, runs whatever logic it needs (file reads, config lookups, regex matching), and outputs either:
...
echo '{"tool_name":"Write","tool_input":{"file_path":"<path>","content":"<content>"}}' \
  | python3 ~/.codex/plugins/local/hookify-plugin/hooks/pretooluse.py
```
