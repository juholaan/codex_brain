...
data_sources:
  ...
  session_captures: on        # ⚙️ Meta/Session Captures.md — quotes from your own Codex sessions
...
# If your AGENTS.md states a journal filename rule, keep this field in sync with it.
...
- **Session Captures** — quotes you typed yourself in your own Codex sessions.
...
One calendar day is still one entry regardless of format (the skill resumes and updates the same file). If your `AGENTS.md` states a filename rule, set this field to match it — `AGENTS.md` is the tiebreaker if they ever disagree.
