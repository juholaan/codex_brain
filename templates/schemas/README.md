...
A worktree-scoped record of one Codex session. Lives in `⚙️ Meta/Sessions/`. Carries date, label, and worktree slug. Consumed by `aggregate-sessions.py`, which rebuilds `Last Session.md` from the most recent N session files.
