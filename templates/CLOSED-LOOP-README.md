...
2. **Episodic memory accumulates.** Each Learning carries
   ...
   back to the originating Codex session. Files are append-only. The hook
...
this stanza to `~/.codex/hooks.json` or the project-local
`.codex/hooks.json`:
...
- Default to the **hourly cron** entry. It is zero-config, runs without
  ...
  operators who batch their Codex sessions.
