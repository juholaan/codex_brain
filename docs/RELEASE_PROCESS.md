...
2. **`--plugin-url` quick-try** for existing Codex 2.1.129+ users — loads the plugin (skills, commands, agents) for the current session only against an existing vault. Frame as evaluation, not substitute. Documented in [`README`](../README.md#quick-try-existing-claude-code-users).
...
3. **Build staged tree** — `rsync` to `/tmp/ai-brain-starter`, excluding `.git`, `.github`, `node_modules`, `__pycache__`, `.venv`, `*.bak`, `.DS_Store`, secrets (`.env`, `.env.*`, `*.key`, `*.pem`, `*.pfx`, `*.p12`, `secrets.json`, `.zsh_secrets`), per-vault state (`.codex/hooks.json`, `.promote-state.json`, `.driftignore`). `.env.example` explicitly included.
