...
**Default:** If unsure, use `priority:: 2`. Codex will re-sort during weekly reviews.
...
Add this to your AGENTS.md so Codex auto-fixes missing fields:

> **To-do field lint.** Every `- [ ]` line in Team To-dos MUST have three inline fields: `[owner:: X]`, `[area:: X]`, and `[priority:: N]`. When Codex touches Team To-dos for any reason, scan all unchecked task lines. If any are missing fields, add them using the prioritization framework before saving. When extracting meeting action items via `$meeting-todos`, the skill must add fields to every new task.
...
Every time Codex touches the to-do file (adding or completing a task), it runs a lightweight priority check. This keeps priorities current without requiring manual weekly reviews to catch drift.
