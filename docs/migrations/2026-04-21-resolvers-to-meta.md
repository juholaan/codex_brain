...
3. Update AGENTS.md rule 3 (or wherever you reference RESOLVERs) to point at the new location. The 2026-04-09 `resolver-md.md` migration doc is superseded by this one.

## Trade-off to know about
You lose the "in-your-face" reminder effect of seeing the resolver when you open the folder. Mitigation: the AGENTS.md rule pointing at `⚙️ Meta/Folder Resolvers/` is read every session, so Codex still knows to check before creating. If you find yourself (or Codex) creating mis-filed notes despite this, you can symlink each resolver back into its folder or re-centralize per your preference.
