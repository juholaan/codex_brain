...
On next pull, Codex should detect this migration and ask two questions:
...
Take their answer and update their `~/.agents/skills/daily-journal/SKILL.md`:
...
- If they don't: update their `~/.agents/skills/daily-journal/SKILL.md` to remove or skip Step 2.5

### Auto-detection

The auto-update hook should trigger this on pull. To detect whether this migration has already been applied, check for the string `"question 6 from setup"` in `~/.agents/skills/daily-journal/SKILL.md`. If present, the migration was already applied. If not, ask the two questions above.
