...
A new rule (#15 under `## Obsidian Rules`) is added to AGENTS.md for every new vault. No setup-time opt-in question — the rule is always installed, and the disclosure happens **inline every time the rule fires**, not during setup. The user opts out the moment they decline once, and the opt-out is permanent.
...
2. Register the scheduled task via whichever mechanism your Codex has
3. Append rule #15 to your `AGENTS.md` (no opt-in question — the rule is always added; disclosure happens inline when the rule actually fires, not during setup)
...
Open your vault's `AGENTS.md`, find the `## Obsidian Rules` section, and add the block below as rule #15 (renumber later rules if you have more than 14). The rule text is in `SKILL.md` under "Phase 16: Add Obsidian Power Rules to AGENTS.md" — copy rule 15 from there verbatim. It covers the 12 corporate-event categories (strategic, procurement, marketing & clients, conferences, incentives, retreats, internal HR, trade shows, technical training, hybrid/digital, hospitality, special high-impact) with the disclosure text baked in and the permanent-opt-out mechanism.
...
- The auto-update hook now runs `sync-skills.sh` deterministically instead of telling Codex to guess what to copy

**If your `.codex/hooks.json` was written before today,** it has the OLD auto-update hook command that does a natural-language "copy any updated skills" instruction. After you pull, the new `sync-skills.sh` will exist in the repo but your old hook won't call it. You need to do **one** of the following exactly once, then you're permanently in sync:

### Easiest — let Codex do it for you after the pull

After the auto-update hook pulls the new commits on your next session start, just tell Codex:

> "Run `bash ~/plugins/codex-brain-starter/scripts/sync-skills.sh` to sync the skill folders, and then update my `.codex/hooks.json` to match the new `hooks.json` in the repo."
...
2. Diff `~/plugins/codex-brain-starter/hooks.json` against your vault's `.codex/hooks.json` and update the hooks section so the new auto-update+sync hook is in place for future sessions.

### Manual — if you prefer to do it yourself

```bash
# 1. Sync the skills (backs up customizations before overwriting)
bash ~/plugins/codex-brain-starter/scripts/sync-skills.sh

# 2. Review what was backed up
cat ~/plugins/codex-brain-starter/.sync.log
...
#    ~/plugins/codex-brain-starter/hooks.json
#    into:
#    [YOUR_VAULT]/.codex/hooks.json
...
**Daily trigger:** ask Codex to delete the scheduled task: *"Remove the `daily-journal-reminder` scheduled task."* Or remove the cron line from `crontab -e` if you installed it via cron. No vault data is affected.

**Onde rule:** delete rule 15 from your AGENTS.md `## Obsidian Rules` section. That's it.
