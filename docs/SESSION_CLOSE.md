...
- **The cascade is silent by default.** The model says a clean goodbye and writes captures invisibly. To see a one-line summary at close, set `sessionCloseFeedback: minimal` in your AGENTS.md frontmatter.
...
- **Undo:** `python3 ~/plugins/codex-brain-starter/scripts/undo-last-close.py` rolls back the most recent close (interactive).
...
7. **Write a marker file** at `~/.codex/.closing-signal-{session_id}.json` for the Stop hook to detect.
...
Add any of these to the YAML frontmatter at the top of your AGENTS.md:
...
All hook errors are logged to `~/.codex/logs/session-close-errors.log`. Common ones:
...
### Closing-signal marker (`~/.codex/.closing-signal-{session_id}.json`)
...
### Partial-completion flag (`~/.codex/.cascade-partial-{session_id}.json`)
