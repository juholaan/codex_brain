...
The first-party skills that ship with AI Brain Starter. Every one is a markdown file you can read in a few minutes; install puts each where Codex can find and trigger it. Most are invoked with a slash command (`$journal`, `$graphify`, `$coach`); a few auto-fire on the right cue.
...
- **diagnose** — self-check against an installed vault. Verifies AGENTS.md, the Meta folder, skills, hooks, and the journal index; prints a green/yellow/red report.
