...
| Audit logging | Git commit history | Append-only audit log per Codex session: who, what file, what tool call, what time, what client agent. Streamed to client SIEM (Splunk, Datadog, Sumo) |
...
│  ─ Codex reads vault from local clone                  │
...
Every Codex session writes an append-only audit event per tool call:
