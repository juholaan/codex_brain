...
How to make Codex remember things across sessions — not just the contents of your vault, but **how you work, what you've corrected, what's load-bearing context**.

This is one of the most underrated patterns in this whole setup. AGENTS.md is what Codex reads at the start of every session. The memory system is what Codex **writes to** when it learns something it should remember next time. Together they turn Codex from a stateless assistant into a colleague who actually accumulates context.

---

## The problem this solves

Without memory, every Codex session starts cold:

- You correct a mistake. Next session, Codex makes the same mistake.
- You explain that a specific teammate is your co-founder (not a direct report). Next session, Codex treats them as a generic team member.
- You discover that one of your scripts has a footgun. Next session, Codex steps on it again.
- You give nuanced feedback ("don't summarize at the end of every response, I can read the diff"). Next session, Codex summarizes.

AGENTS.md catches the broad rules. The memory system catches the **specific accumulated knowledge** that's too narrow for AGENTS.md but too important to keep re-learning.
...
```
~/.codex/projects/<project-id>/memory/
```

(The `<project-id>` is the URL-encoded path of your vault — Codex creates this automatically the first time you run a session in a directory.)
...
`MEMORY.md` is loaded into Codex's context at session start (lines 1-200). Individual memory files are loaded on demand when their description is relevant.
...
Information about your role, goals, responsibilities, and knowledge. Helps Codex tailor explanations to your level and frame work in terms you already understand.
...
Guidance about how to approach work — both what to avoid and what to keep doing. **Save from corrections AND from validated wins.** If you only save corrections, Codex will avoid past mistakes but drift away from approaches you've already validated, growing overly cautious.
...
**Format:** lead with the rule, then `**Why:**` (the reason — often a past incident), then `**How to apply:**` (when this kicks in). Knowing *why* lets Codex judge edge cases instead of blindly following the rule.
...
- **Anything already documented in AGENTS.md** — that file is loaded every session anyway
...
The `description` field is critical — it's what Codex scans to decide whether to load the full memory file. Vague descriptions like "stuff about teammate X" are useless. Specific descriptions like "teammate X is co-founder, not a direct report, works at a partner institution" tell Codex exactly when to load it.
...
**Before adding, check:**
- Is this already in AGENTS.md? If yes, skip — AGENTS.md loads every session anyway; duplicating it in memory pays the cost twice.
...
## Memory vs AGENTS.md vs Tasks vs Plans

There are several persistence layers in this setup. They serve different purposes:

| Layer | Lifetime | Loaded when | Best for |
|---|---|---|---|
| **AGENTS.md** | Indefinite, broad scope | Every session start | Universal rules: tone, file structure, vault conventions, accountability rules |
...
Project-level memory (`~/.codex/projects/.../memory/`) is tied to ONE Codex account and ONE project path. If the user switches Codex accounts (personal, business, team), memories from other accounts are invisible. Memory is a convenience index for fast recall, not the canonical store.

**Rule: Never store something ONLY in memory.** Every memory should be backed by a durable file that any Codex session can read:

| What you learned | Back it up in |
|---|---|
| Personal preference or rule | Vault AGENTS.md (Preferences section) or `rules/` files |
...
When saving a memory, ask: *"If this user opens the vault from a different Codex account tomorrow, will this information be available?"* If no, also put it in a vault file.
...
|---|---|---|
| **Local agent memory** (`~/.codex/projects/<key>/memory/`, a real dir) | This machine + this Codex account ONLY. Not other accounts, not other tools (e.g. Codex), not CI, not teammates, not a retrieval runtime. | Who you are, your preferences, a quirk of THIS machine's tooling, a single repo's CI gotcha, current project state — genuinely-local facts. |
...
- **Team lesson trapped in local memory.** You learn a reusable engineering principle, write it to `~/.codex/.../memory/`, and it never reaches another account, Codex, your teammate, CI, or the runtime — so it can't change how the team works. A write-time guard (`hooks/warn-learning-to-tool-private-memory.py`, installed at user level) NUDGES when a learning-shaped file lands in a *real* tool-private memory dir. It never blocks — local memory is still the right home for local facts.
- **Model-general guard stuck in one person's `~/.codex`.** A guard that should protect every install lives only on the maintainer's machine. If it's model-general, it belongs in the substrate, activated by the installer and proven by a fresh-install smoke — not just `~/.codex`.

One nuance on durability vs reach: if your `~/.codex/projects/<key>/memory` dir is a SYMLINK into your vault (the installer wires this), you've solved durability ACROSS TOOLS on this machine — but a *local-only-git* vault still doesn't reach another GitHub account, CI, or a teammate. Cross-account / teammate reach needs a git-remote-backed shared brain, not just a symlink.
...
1. **Set up the directory** at `~/.codex/projects/<your-project>/memory/` with an empty `MEMORY.md`.
2. **Tell Codex about it** by adding a line to your AGENTS.md: *"Use the memory system at `~/.codex/projects/<your-project>/memory/` to record corrections, project state, and discoveries. Read MEMORY.md at session start."*
3. **Be explicit when you correct things.** Instead of *"no, don't do it that way"*, say *"don't do it that way — save a feedback memory: [reason]"*. This makes Codex actually write the memory file instead of just acknowledging the correction.
4. **Quarterly memory hygiene.** Set a calendar reminder.

After ~20 sessions you'll notice Codex stops making the same mistakes. After ~50 sessions you'll notice it remembers nuances about how you work that you don't even remember telling it. That's the system working.
