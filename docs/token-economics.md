...
- **MCP server tool schemas** are often deferred in Codex (loaded on search rather than always-on) — the harness-level win that makes MCP servers feasible at scale. The deferred-tool **name list** still contributes a smaller per-session cost, so unused MCP servers in `.codex/config.toml` should be pruned.
- **`SessionStart` hooks** inject context at every session start. Measure the injection size before adding a new hook. Cache long-running checks (24h TTL pattern works well): `echo "$msg" > ~/.codex/.<my-hook>-cache` and skip the network call when the cache is fresh.
...
- **`/usage` in Codex 2.1.149+**: per-category breakdown for skills, subagents, plugins, and per-MCP-server cost. Use it to cross-check the script's numbers.
...
- A note in your AGENTS.md or AGENTS.md pointing future sessions at this discipline.
