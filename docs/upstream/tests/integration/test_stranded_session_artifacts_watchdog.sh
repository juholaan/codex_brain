...
#
# Complements surface-orphan-codex-branches.py: that hook catches the
...
# This suite invokes the hook directly, and the hook resolves ~/.codex. Without a
# sandbox it therefore rewrites the DEVELOPER'S real ~/.codex/settings.json on every
...
echo "stranded session body" \
  > "$VAULT/.codex/worktrees/wt-stranded/Meta/Sessions/2026-05-17T12-00-wt-stranded.md"
cd "$VAULT"
OUT="$(run_watchdog)"
if [ -z "$OUT" ]; then
...
# case belongs to the sibling hook surface-orphan-codex-branches.py.
cd "$VAULT/.codex/worktrees/wt-stranded"
