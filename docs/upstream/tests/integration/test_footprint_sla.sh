...
entries = [{"type": "command",
            "command": f"python3 ~/plugins/codex-brain-starter/hooks/evil-{i}.py 2>/dev/null || echo '{{}}'"}
           for i in range(30)]
...
import json, sys
entries = [{"type": "command",
            "command": f"python3 ~/plugins/codex-brain-starter/hooks/h-{i}.py 2>/dev/null || echo '{{}}'"}
           for i in range(3)]
json.dump({"hooks": {"PreToolUse": [{"matcher": "Write|Edit", "hooks": entries}]}},
          open(sys.argv[1], "w"))
PY
...
# Axis D-live measures an install's ACTUAL ~/.codex/settings.json (owned + unowned +
