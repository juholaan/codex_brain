...
# file (<vault>/⚙️ Meta/rules/meeting-workflow.md), not just to AGENTS.md.
#
...
# rule — but appended it to AGENTS.md instead of overwriting the vault
...
#      into AGENTS.md (the prior bug-source).
...
grep -qiE "do NOT duplicate|not duplicate the rule body|two near-identical bodies|drift" "$PHASE" || \
    fail "Phase 11 must warn against duplicating the rule body into AGENTS.md (the source of the original bug)"

echo "PASS: phase-11-external-tools.md instructs Phase 11 to write customized rule to vault file (not duplicate into AGENTS.md)"
