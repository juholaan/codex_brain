...
# inside <vault>/.codex/worktrees/<slug>/, cwd IS the worktree, so the
...
# touches the real ~/.codex. Exit 0 = pass, 1 = fail with detail on stderr.
...
VAULT="$TMP/vault"
WT="$VAULT/.codex/worktrees/test-slug"
