...
# "Personal scope" Codex substrate IS this repo — bootstrap.sh clones
# ai-brain-starter@main and install-hooks-user-level.py wires the canonical
# root hooks.json into ~/.codex/settings.json. So whatever this hooks.json
...
data.setdefault("hooks", {}).setdefault("SessionStart", []).append(
    {"hooks": [{"type": "command",
                "command": f"python3 ~/plugins/codex-brain-starter/hooks/{needle}"}]}
)
