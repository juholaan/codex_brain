  ...
  "hooks": {
    "UserPromptSubmit": [
      { "hooks": [
        ...
        { "type": "command", "command": "python3 ~/plugins/codex-brain-starter/scripts/email-gate-hook.py 2>/dev/null || echo '{}'" },
...
cat > "$SETTINGS" <<JSON
{
  "hooks": {
    "UserPromptSubmit": [
      { "hooks": [
        { "type": "command", "command": "python3 ~/plugins/codex-brain-starter/scripts/post-update-email-ask.py 2>/dev/null || echo '{}'" },
        { "type": "command", "command": "$CUSTOM" }
      ] }
    ]
  }
