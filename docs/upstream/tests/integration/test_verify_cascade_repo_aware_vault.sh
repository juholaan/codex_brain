...
cat > "$REPO_NORUN/AGENTS.md" <<'EOF'
## Session End — capture cascade
EOF
WT_NORUN="$REPO_NORUN/.codex/worktrees/slug1"
mkdir -p "$WT_NORUN"
...
WT_PASS="$REPO_PASS/.codex/worktrees/slugpass"
mkdir -p "$WT_PASS"
...
cat > "$REPO_INCOMPLETE/AGENTS.md" <<'EOF'
## Session End — capture cascade
EOF
: > "$REPO_INCOMPLETE/⚙️ Meta/scripts/session-close-runner.sh"
WT_INCOMPLETE="$REPO_INCOMPLETE/.codex/worktrees/sluginc"
