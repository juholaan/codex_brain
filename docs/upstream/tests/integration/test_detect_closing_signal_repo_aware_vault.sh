...
#
# Bug class: VAULT_ROOT is commonly set once, globally, in Codex's
...
# vault-shaped repo (its own AGENTS.md declaring its own "Session End" /
...
#   4. FALLBACK PRESERVED — cwd in an untracked location (no AGENTS.md
...
#      AGENTS.md uses a heading that does NOT declare a Session End/Close
...
# never touches the real ~/.codex and so the walk-up's $HOME boundary is
...
# personal vault) — a real vault shape, but its AGENTS.md heading is
...
cat > "$DEFAULT_VAULT/AGENTS.md" <<'EOF'
# Default Vault
...
# vault (e.g. mycelium-vault) — its own AGENTS.md declares its own cascade.
REPO_VAULT="$TMP/repo-vault"
mkdir -p "$REPO_VAULT/Meta/Sessions" "$REPO_VAULT/Meta/Decisions"
cat > "$REPO_VAULT/AGENTS.md" <<'EOF'
# Repo Vault
...
EOF
REPO_WT="$REPO_VAULT/.codex/worktrees/test-slug"
mkdir -p "$REPO_WT/Meta/Sessions" "$REPO_WT/Meta/Decisions"
cp "$REPO_VAULT/AGENTS.md" "$REPO_WT/AGENTS.md"   # a real worktree carries the full tree

# LOWERCASE_VAULT: same as REPO_VAULT but with a lowercase "Session end"
# heading, matching the real second confirmed case (a team-folder AGENTS.md
# used "## Session end — capture cascade", lowercase "end").
LOWERCASE_VAULT="$TMP/lowercase-vault"
mkdir -p "$LOWERCASE_VAULT/Meta/Sessions"
cat > "$LOWERCASE_VAULT/AGENTS.md" <<'EOF'
## Session end — capture cascade
EOF

# UNTRACKED: no AGENTS.md anywhere in its ancestry (up to $TMP == $HOME below).
...
assert_meta_dir \
  "untracked cwd (no AGENTS.md in its ancestry) falls back to VAULT_ROOT" \
  "$MARKER4" "$DEFAULT_VAULT/Meta"

# --- 5. FALLBACK PRESERVED: default vault's own AGENTS.md doesn't self-match
