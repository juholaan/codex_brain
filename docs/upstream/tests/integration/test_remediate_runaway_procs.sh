...
# them. CLASS 2 reaps a stuck ~/.codex/hooks/ python process, gated by path
...
# Assertions:
#   POSITIVE control:
#     1. A high-age, high-CPU python process under ~/.codex/hooks/ IS reaped.
...
HOOKS = os.path.expanduser("~/.codex/hooks/")
