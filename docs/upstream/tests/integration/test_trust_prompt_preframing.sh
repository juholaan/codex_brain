#!/usr/bin/env bash
# Regression test: the install must pre-frame Codex's built-in trust
# prompt for third-party plugins, marketplaces, and MCP servers.
#
# Bug class: a non-technical installer hits Codex's trust prompt
...
must_contain "$README" \
  "README is missing the English 'What Codex will ask you' section" \
  "What Codex will ask you"
must_contain "$README" \
  "README is missing the Spanish trust-prompt section" \
  "Qué te va a preguntar Codex"
...
#    a fresh Codex reading the README would respond to "install this for me"
...
# 7. The README + phase-00 must pre-frame Codex's command-approval prompt
...
if [ "$FAILED" -gt 0 ]; then
  ...
  echo "The install must keep getting ahead of Codex's built-in trust prompt:" >&2
