...
#      NOT an opt-in one) into a throwaway ~/.codex, preserves a user's
...
# B2: the REAL installer copies the default rule into a throwaway ~/.codex and
...
if [ -f "$H1/.claude/$DEFAULT_RULE" ]; then
  ok "installer activated the default rule into ~/.codex"
else
