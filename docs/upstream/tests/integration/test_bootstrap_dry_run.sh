...
sandbox_home "$TMP"
touch "$HOME/.codex/.ai-brain-starter-email-on-file"

# Pre-create a fake SKILL_DIR so the bootstrap finds itself.
mkdir -p "$HOME/plugins/codex-brain-starter"
# Symlink the test runner's repo into the SKILL_DIR position so bootstrap
# can find templates, skills/, scripts/, etc.
rmdir "$HOME/plugins/codex-brain-starter"
ln -s "$REPO_ROOT" "$HOME/plugins/codex-brain-starter"
