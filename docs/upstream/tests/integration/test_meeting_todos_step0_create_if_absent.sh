...
# nothing matched. The meeting extraction work was already done — Codex
...
# test (no shell behavior to exercise — SKILL.md is read by Codex),
...
# 2. Common-paths-first guidance present (cheap before falling back to AGENTS.md).
grep -qF "🏠 Home/✅ Get to-do.md" "$SKILL" || \
    fail "Step 0 should name the canonical to-do path '🏠 Home/✅ Get to-do.md'"

# 3. AGENTS.md fallback path documented.
grep -qiF "vault \`AGENTS.md\`" "$SKILL" || \
    fail "Step 0 should fall back to reading vault AGENTS.md for a hint"
...
grep -qiE "On no.*(stop|silent)" "$SKILL" || \
    fail "Step 0 should explicitly tell Codex to STOP on 'no' rather than continue and lose the extraction"
