...
# fake settings.json and writes a fake state file -- the real ~/.codex is never
...
write_settings "$H1" \
  'python3 ~/.codex/hooks/foo.py' \
  '~/.codex/hooks/bar.sh' \
  'python3 ~/.codex/hooks/baz.py --flag'
run_guard "$H1"
...
write_settings "$H1" \
  '/usr/bin/python3 /home/u/.codex/hooks/foo.py' \
  '[ -f ~/.codex/hooks/bar.sh ] && ~/.codex/hooks/bar.sh' \
  "python3 ~/.codex/hooks/baz.py --flag 2>/dev/null || echo '{\"continue\":true}'"
run_guard "$H1"
...
write_settings "$H2" \
  '/usr/bin/python3 /home/u/.codex/hooks/foo.py' \
  '[ -f ~/.codex/hooks/bar.sh ] && ~/.codex/hooks/bar.sh' \
  "python3 ~/.codex/hooks/baz.py --flag 2>/dev/null || echo '{}'"
run_guard "$H2"
...
write_legacy_snapshot "$H3" \
  'python3 ~/.codex/hooks/foo.py' \
  'python3 ~/.codex/hooks/baz.py --flag'
write_settings "$H3" 'python3 ~/.codex/hooks/foo.py'
run_guard "$H3"
...
H4="$(newhome)"
write_settings "$H4" 'python3 ~/.codex/hooks/foo.py'
run_guard "$H4"                         # baseline
write_settings "$H4" \
  'python3 ~/.codex/hooks/foo.py' \
  'python3 ~/.codex/hooks/new.py'
run_guard "$H4"
