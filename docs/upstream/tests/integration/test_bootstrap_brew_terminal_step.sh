...
# non-interactive shell (the user pasted the install prompt into Codex,
...
# the web guide may have installed it where Codex's non-login shell can't
...
{
  echo 'SKILL_DIR="/home/u/plugins/codex-brain-starter"'
  echo 'hdr(){ printf "%s\n" "$*"; }'
...
echo "$EN_OUT" | grep -q 'bash "/home/u/plugins/codex-brain-starter/bootstrap.sh"' \
...
# reinstall — otherwise a Terminal-installed brew that just isn't on Codex
