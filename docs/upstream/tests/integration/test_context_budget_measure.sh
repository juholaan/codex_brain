...
# WARNS on an over-ceiling synthetic global AGENTS.md. Self-contained; no real ~/.codex.
set -euo pipefail
...
# 3. Black-box: warns on an over-ceiling global AGENTS.md.
python3 -c "open('$TMP/.claude/AGENTS.md','w').write('x'*41000)"
rm -f "$TMP/.claude/.context-budget-baseline.json" "$TMP/.claude/.context-budget-last-warn"
OUT="$(printf '{"cwd":"%s"}' "$TMP" | run_sandboxed "$TMP" env CLAUDE_PROJECT_DIR="$TMP" python3 "$HOOK")"
if echo "$OUT" | grep -q 'over the'; then
  echo "OK: warns on over-ceiling global AGENTS.md"
else
