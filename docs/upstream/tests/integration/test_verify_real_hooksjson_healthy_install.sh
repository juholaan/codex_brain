...
#   - ~/plugins/codex-brain-starter/... home copies present (every install has these)
#   - vault-content hooks under <vault>/⚙️ Meta/scripts/... present (phase-05 wires them)
#   - the skill repo is NOT cloned into <vault>/plugins/codex-brain-starter/, so the
...
# hook scripts into the developer's real ~/.agents/skills (MYC-3536).
...
def is_skill_in_vault(p):
    ...
    return p.startswith(vres) and "/plugins/codex-brain-starter/" in p
...
victim = Path(os.path.expanduser("~/plugins/codex-brain-starter/hooks/log-skill-usage.py"))
