...
# into ~/.codex/settings.json when there is no vault yet (no --vault-path).
#
...
#      (detect-closing-signal.py, which has a ~/.codex fallback) ARE still
...
# The closing-signal hook carries a ~/.agents/skills fallback, so it works with
