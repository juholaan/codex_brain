...
# (only the ~/.codex home copy does). The verifier flagged the missing vault
...
#
# Asserts:
#   (a) a detect-closing-signal-shaped `||` chain where ONLY the ~/.codex home
...
# Throwaway HOME so we never touch the real ~/.codex/settings.json.
TMP_HOME="$(mktemp -d)"
...
SKILL_DIR="$TMP_HOME/plugins/codex-brain-starter"
mkdir -p "$SKILL_DIR/hooks"

# The two copies the detect-closing-signal fallback chain references:
#   HOME_COPY  — ~/.agents/skills/... — guaranteed present on every install.
#   VAULT_COPY — inside the vault — legitimately absent on a vault install.
HOME_COPY="$SKILL_DIR/hooks/detect-closing-signal.py"
VAULT_COPY="$TMP_HOME/MyVault/plugins/codex-brain-starter/hooks/detect-closing-signal.py"
