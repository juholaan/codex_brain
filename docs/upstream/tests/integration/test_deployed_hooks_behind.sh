...
# fails OPEN: on a silent failure the checkout is current but ~/.codex/settings.json
...
# by whatever bare skill copies happen to live in the real ~/.agents/skills.
...
inject_hook "$TMP/retired.json" "UserPromptSubmit" \
  "python3 ~/plugins/codex-brain-starter/scripts/email-gate-hook.py 2>/dev/null || true"
...
{ "hooks": { "UserPromptSubmit": [ { "hooks": [
  ...
  { "type": "command", "command": "python3 ~/plugins/codex-brain-starter/hooks/log-skill-usage.py 2>/dev/null || echo '{}'" }
] } ] } }
JSON
