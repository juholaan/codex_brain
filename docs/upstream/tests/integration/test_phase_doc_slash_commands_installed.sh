...
# never copied to ~/.agents/skills/ because bootstrap.sh's install loop
...
for skill in "${REQUIRED_SKILLS[@]}"; do
  ...
  # copies files to ~/.agents/skills/). We scan for the literal skill
  ...
  # Codex palette. This was the surface bug behind the 2026-05-14
  ...
  if [ ! -f "$COMMANDS_DIR/$skill.md" ]; then
    ...
    echo "  Without this file, /$skill won't appear in the Codex slash command palette." >&2
