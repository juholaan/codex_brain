...
Also new: **`docs/RECOMMENDED_SKILL_OVERRIDES.md`** — a portable starter recipe for the `skillOverrides` setting that landed in Codex 2.1.129. Sharper auto-routing without disabling skills you might still need.
...
Existing Codex 2.1.129+ users can now try AI Brain Starter skills against an existing vault without running the full bootstrap:
...
New skill for onboarding companies that already have history in Slack, Notion, GDocs, or any markdown corpus. Reads the dump, finds recurring decision phrases, owner-of-process patterns, and template-shaped rituals, and emits draft `AGENTS.md` entries, draft hookify rules, and draft skill scaffolds.
...
## 2026-04-23 — Codex v2.1.118 improvements

Upstream Codex shipped several improvements worth knowing about.
...
- **Auto-update is now weekly, not every session.** The hook that checks GitHub for skill updates used to run on every session start (slow, network round-trip every time you opened Codex). Now it runs at most once per 7 days, gated by a timestamp at `~/.codex/.ai-brain-starter-last-update`. Sessions start faster.
...
- New `docs/TOKEN_OPTIMIZATION.md`: where Codex burns tokens on overhead (5K-20K per message before you type anything) and six fixes that cut 50-70%.
...
`AGENTS.md` and `templates/generated/agents-md-template.md` ship a new rule: **never run `git add -A`, `git add .`, or unscoped `git status` in a large vault.** Always pass explicit file paths.

The problem this fixes: Codex was running `git add -A` inside large Obsidian vaults during session close, walking 60K+ files, locking `.git/index.lock` for 10+ minutes, and burning context while polling for progress. Rules in AGENTS.md prevent this from happening to new vaults.
...
- `hooks/file-changed-settings.sh`: validates `.codex/hooks.json` and `.codex/config.toml` on every write.
- `hooks/rotate-logs.sh`: rotates hook logs at 500KB, keeps 3 gzipped generations. Prevents unbounded log growth.
- `hooks/claude-scheduled-runner.sh`: headless Codex launcher for scheduled tasks.
...
- Slash commands now expose `argument_hints` so Codex shows you what arguments are accepted before you press enter.
