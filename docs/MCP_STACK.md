...
- [parse-mcp](https://github.com/adelaidasofia/parse-mcp) — markitdown / Docling / LlamaParse router. Plus an `interpret` tool that pipes parsed markdown into Codex with prompt caching.
...
1. Vault first. Follow the install guide. Get the directory structure + skills + AGENTS.md scaffolding in place.
...
3. Wire each MCP into `<vault>/.codex/config.toml`. Restart Codex. Verify with `codex mcp list`.
...
3. Generic utility — useful to anyone running Codex, not just one author's stack.
