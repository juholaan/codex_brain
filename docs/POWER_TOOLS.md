...
The third-party Codex skills, MCP servers, and Obsidian plugins that make this setup actually work.
...
These get installed into `~/.agents/skills/` and become available as `/skill-name` commands.
...
**Configuration:** add `closingSignals.custom: [...]`, `closeDetection: hybrid`, or `sessionCloseFeedback: minimal` to your AGENTS.md frontmatter. Full reference in [`docs/SESSION_CLOSE.md`](SESSION_CLOSE.md).

**Recovery:** `python3 ~/plugins/codex-brain-starter/scripts/recover-last-close.py` (resume after partial close). `python3 ~/plugins/codex-brain-starter/scripts/undo-last-close.py` (rollback).

**Testing:** `python3 ~/plugins/codex-brain-starter/scripts/test-closing-signals.py` runs 74 fixtures.
...
**Why it matters for a vault:** Once you have ~500 notes, you stop being able to hold the whole structure in your head. Graphify gives you a map. The `GRAPH_REPORT.md` becomes the **first thing Codex reads** for any strategic question — way faster and more accurate than reading individual files. On a 4,700-file personal vault, it cuts cross-concept research from "read 10 files" to "read one report."
...
The Codex skill (in `~/.agents/skills/graphify/`) wraps the CLI with optimization scripts. `$setup-brain` Phase 0 installs both. The full pipeline + lessons learned is in [`skills/graphify/RUNBOOK.md`](../skills/graphify/RUNBOOK.md).

**Trigger:** `$graphify <folder>` to build, `$graphify <folder> --update` to incrementally update.

**Source:** [graphifyy on PyPI](https://pypi.org/project/graphifyy/) (the underlying package). The Codex skill in this repo wraps it with vault-specific optimizations.
...
**Install:**
```bash
git clone https://github.com/adelaidasofia/humanizer.git ~/.agents/skills/humanizer
```
...
**Install:** Adds to Codex via the [devon-claude-skills marketplace](https://github.com/devonjones/devon-claude-skills):
```bash
...
Each skill in `skills/<name>/` is auto-discovered by Codex. After cloning, restart Codex so the SKILL.md files load.
...
The substrate ships memory, voice, vault, and session lifecycle. These vendor-published bundles cover the engineering-side skills a serious build benefits from. `bootstrap.sh` installs them via Codex's native plugin marketplace mechanism (skip with `SKIP_VENDOR_SKILLS=1` for air-gapped installs). Each vendor maintains their own SKILL.md per platform; we do not fork, repackage, or redistribute.

**Install path: `claude plugin marketplace add <repo>` + `claude plugin install <name>@<marketplace>`.** Plugin install is the ONLY path that registers nested SKILL.md files for Codex's auto-discovery. Raw `git clone` into `~/.agents/skills/` does NOT work for bundles with nested SKILL.md (the most common shape) because Codex auto-discovery only finds top-level SKILL.md. Codified 2026-05-10 after audit caught 7 cloned bundles invisible to Codex despite being on disk.
...
**Install:** automatic via `bootstrap.sh`. Manual: `claude plugin marketplace add getsentry/sentry-skills && codex plugin install sentry-skills@sentry-skills`.
...
**License caveat:** CC-BY-SA-4.0 is copyleft on documentation. Cloning into your own `~/.agents/skills/` is fine. **Forking the documentation into a derived work requires keeping the same license.** Do not bundle Trail of Bits content into MIT-licensed downstream repos without preserving CC-BY-SA-4.0.

**Install:** automatic via `bootstrap.sh` (installs 7 relevant plugins from the marketplace bundle: insecure-defaults, sharp-edges, property-based-testing, static-analysis, testing-handbook-skills, differential-review, ask-questions-if-underspecified). Manual: `claude plugin marketplace add trailofbits/skills && codex plugin install <name>@trailofbits` per plugin.
...
**Install:** automatic via `bootstrap.sh`. Manual: `claude plugin marketplace add stripe/agent-toolkit && codex plugin install stripe@stripe`.
...
**Install:** automatic via `bootstrap.sh`. Manual: `claude plugin marketplace add cloudflare/skills && codex plugin install cloudflare@cloudflare`.
...
**Install:** automatic via `bootstrap.sh`. Manual: `claude plugin marketplace add AgriciDaniel/claude-seo && codex plugin install claude-seo@agricidaniel-claude-seo`.
...
**Important:** this is a CLI TOOL, not a SKILL.md-format skill. It does not auto-load when present in `~/.agents/skills/`. Use it as a command-line generator, then commit the resulting SKILL.md into the appropriate skill directory.
...
**Install:** automatic via `bootstrap.sh`. Manual: `git clone https://github.com/yvgude/lean-ctx.git ~/.agents/skills/lean-ctx`. Then `lean-ctx init --agent claude-code` per upstream docs.
...
**Install:** automatic via `bootstrap.sh`. Manual: `git clone https://github.com/vercel-labs/agent-skills.git ~/.agents/skills/vercel-agent-skills`.

### Catalog source

These adjacents were surfaced via [VoltAgent/awesome-agent-skills](https://github.com/VoltAgent/awesome-agent-skills) (21k stars, MIT, hand-curated). The full catalog covers 1,100+ skills from official teams plus community contributions. The monthly `~/.codex/scheduled/scan-awesome-repos.sh` job watches this and four other awesome-* repos for new entries; reports land in `⚙️ Meta/Awesome Repo Scan/<YYYY-MM>.md`.

---

## Repo-shipped CLI tools

Standalone scripts this repo bundles at `scripts/` that you run directly against any git repo — not Codex skills, no setup required beyond a Python interpreter.
...
**What it does:** A fast, cheap reasoning model good at extraction, classification, summarization, and boilerplate generation. Not a replacement for Codex on judgment-heavy work, but a workhorse for grunt-work text processing.
...
# Test it
./scripts/minimax.sh "Summarize this in 3 bullet points: Codex is a terminal-based AI coding assistant built by Anthropic."
```
...
**Why it matters:** without this, you spend 20 minutes after every meeting transcribing handwritten notes and updating CRM cards. With it, Codex reads the full transcript and does the cascade in one command.
...
### ChatPRD — product specs and PRDs from Codex

**What it does:** [ChatPRD](https://www.chatprd.ai/) is an AI tool purpose-built for product requirements documents. The MCP integration lets Codex create, search, read, and update PRDs in your ChatPRD workspace without leaving the terminal. You can say "create a PRD for the venue search feature" and Codex writes it directly into ChatPRD.
...
**Install:** Add to your vault `.codex/config.toml` (the `.codex/config.toml` file at your vault root, NOT `~/.codex/.codex/config.toml`):
...
Then open Codex and use any ChatPRD tool — it will prompt you to authenticate via OAuth. One-time setup, then it stays connected.
...
Register in your project `.codex/config.toml`:
```json
{
  "mcpServers": {
    "health": {
      "command": "python3",
      "args": ["-m", "main"],
      "env": {"HEALTH_MCP_DB": "~/.codex/health-mcp/health.duckdb"}
    }
  }
}
```

Restart Codex, then run `$ingest-health <path-to-export.zip>` and `health_status()` for a smoke check.
...
The Codex MCP ecosystem is growing fast. Other servers worth adding for a founder workflow:

- **Linear MCP** — issue/project tracking. Lets Codex read issue context and update statuses without you switching tabs.
...
2. **You run a meeting** with Granola recording. Afterward you say *"I just had a meeting with Sara"*. The meeting workflow rule in AGENTS.md fires:
...
5. **Strategic moment:** you ask Codex *"what does my vault say about X?"*. Codex reads `graphify-out/GRAPH_REPORT.md` first (god nodes, communities, hyperedges) instead of grepping individual files. The answer comes with context, not just keyword matches.
...
None of these tools are mine. They're all open source by other people. What `$setup-brain` does is **install them and wire them together** with the right AGENTS.md rules, templates, and folder structure so the whole stack acts like one tool.
