...
`~/.codex/.ai-brain-starter-corporate-manifest.md` for your records.
...
| 3 | Optional email signup + a best-effort install ping | Both off: `EMAIL_GATE_BYPASS=1`, `MYCELIUM_NO_PING=1`, plus Codex telemetry env enforced in `settings.json`. |
| 4 | Auto-updates: a self-update hook pulls the skill repo every ~6 days; Codex auto-updates | Versions pinned: self-update hook disabled via a sentinel; Codex autoupdater off. Updates require a reviewed manual re-run. |
...
every corporate run and writes it to `~/.codex/.ai-brain-starter-corporate-manifest.md`.
...
| `context7` | Codex plugin | Anthropic plugin registry | Documentation lookup. Read-only, no data egress. |
...
The corporate profile enforces these in `~/.codex/hooks.json` (`env` block),
so they persist for every future Codex session, not just the install:

| Variable | Effect |
|---|---|
| `DISABLE_TELEMETRY=1` | No usage telemetry from Codex. |
...
| `DISABLE_AUTOUPDATER=1` | Belt-and-suspenders pin of the Codex CLI version. |
...
- The installer creates `~/.codex/.ai-brain-starter-pinned`. The self-update
...
return to automatic updates, delete `~/.codex/.ai-brain-starter-pinned`.
...
- **A skipped marketplace/plugin:** inside Codex, run
  `/plugin marketplace add <owner/repo>` then `/plugin install <name>`.
- **An external MCP (granola / chatprd):** add it to `~/.codex/.codex/config.toml`. The
...
- [ ] Confirm `~/.codex/hooks.json` `env` carries the six telemetry/pin keys above.
- [ ] Confirm `~/.codex/.codex/config.toml` contains no external URL MCPs you didn't approve.
- [ ] Confirm the vault path is not under a cloud-sync folder.
- [ ] Confirm Obsidian Restricted Mode is on for sensitive vaults.
- [ ] Confirm Obsidian + Codex auto-update are disabled.
