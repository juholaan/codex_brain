...
Your brain is a git repository, and the Codex **Desktop** app has a per-session
...
- **Desktop app:** open the vault with the per-session **worktree box
  UNCHECKED**.
- **Terminal:** `cd /path/to/your/vault && codex` — a plain CLI session is
...
3. **Reclaim the leftover checkouts.** The worktree-hygiene hooks remove ended
   ...
   ```
4. **Confirm green:** `bash ~/plugins/codex-brain-starter/scripts/diagnose.sh
