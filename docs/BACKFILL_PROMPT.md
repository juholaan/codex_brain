...
description: Self-contained prompt for running the health-data backfill across journals in a fresh Codex session
---

# Backfill prompt — health-data into journals (fresh session ready)

Copy everything below the `---PROMPT---` marker into a fresh Codex session. It is self-contained: it explains the goal, the safety constraints, the model routing, and the verification steps. Codex will run the work end-to-end and report back with a summary.

The prompt assumes:
- The `health` MCP (this repo's `services/health-mcp/`) is registered in `.codex/config.toml` and connected.
