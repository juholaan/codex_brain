...
- Codex desktop app installed and signed in (Pro or Max).
...
### Beat 1: open Codex (0:08 to 0:25)

*"This is Codex. I'm signed in. I'm going to set up a brand-new vault by pasting one prompt."*

Cut to Codex app, fresh session, empty input.

### Beat 2: paste the install prompt (0:25 to 0:55)

*"This is the prompt. It tells Codex where to clone the repo, runs the bootstrap script, and starts the setup interview. One paste."*

Show the prompt being pasted from clipboard:

```
Please set up my AI Brain Starter end-to-end in this session. Clone https://github.com/mycelium-hq/ai-brain-starter.git into ~/plugins/codex-brain-starter (git pull if it already exists), run bash ~/plugins/codex-brain-starter/bootstrap.sh, then start the setup interview by running the setup-brain skill. Keep going through every phase without stopping. I shouldn't have to type any commands between steps.
```
...
*"Codex clones the repo. Runs the bootstrap script. Installs Obsidian, Python, the graphify dependencies, the skills, the hooks. I don't type anything. The system installs itself."*
...
*"Now Codex interviews me. Who I am. What I do. What goals matter this quarter. Who the most important people in my life are. The answers shape the vault structure. There's no config file. The whole setup is a conversation."*
...
3. The pre-title card (start cold on the Codex paste).
...
- [ ] Terminal and Codex window pre-sized.
