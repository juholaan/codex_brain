...
description: Self-contained prompt for drafting a Substack article about the health stack (Apple Health / Oura / Fitbit + cycle-aware coaching + journal pairing + auto-trigger chain). Paste into a fresh Codex session. Output: two Substack drafts (English + Spanish) saved to your two publications, NOT published.
---

# Substack article prompt — the health package, for healthy people who are not tech people

Copy everything below the `---PROMPT---` marker into a fresh Codex session. The session must have:
- The user's vault loaded (AGENTS.md auto-loads)
