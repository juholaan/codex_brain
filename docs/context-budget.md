...
turn, forever — your `AGENTS.md` files, `MEMORY.md`, and `CONTEXT.md`.

That layer is the most expensive context in the system, and the one with no natural
guard. Plugin load is measured; MEMORY.md has a cliff guard; but the `AGENTS.md`
...
- **measures** the always-loaded files it can find — global `~/.codex/AGENTS.md`,
  the project's `AGENTS.md`, `MEMORY.md`, the project's `CONTEXT.md` — and reports
  the total in bytes + estimated tokens;
- **warns on a hard per-file ceiling** (default: global `AGENTS.md` over 40 KB ≈ 10K
...
find the project's `AGENTS.md` / `MEMORY.md` / `CONTEXT.md`; run from a non-project
...
python3 ~/.codex/hooks/context-budget-measure.py --report
...
python3 ~/.codex/hooks/context-budget-measure.py --accept

# Prove it fires/stays-silent correctly (positive + negative control):
python3 ~/.codex/hooks/context-budget-measure.py --self-test
```
...
- A `AGENTS.md` rule with multi-paragraph "why" + an incident retelling → compress to
