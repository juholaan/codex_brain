...
You are running a deep optimization on an existing Obsidian vault. The user has already set up the basics (folder structure, AGENTS.md, context layer) — either through `$setup-brain` or manually. Now they want the full treatment.
...
2. **AI Chat Import & Cleanup** — export and import your ChatGPT/Codex/Gemini history, keep the gold, delete the noise (30-60 min)
...
10. **Skill & Routing Health Check** — verify all skills work, paths resolve, AGENTS.md routing is complete (10 min)
...
- **Codex (claude.ai):** Settings → Account → Export Data → zip file
...
### Step 3: Link
Add links to domain summaries from AGENTS.md and 00 Start Here if they aren't already there.
...
"Let me make sure all your skills are working and your AGENTS.md has the right routing entries."

### Step 1: Verify skill paths
Check that every skill file referenced in AGENTS.md actually exists:
...
ls ~/plugins/codex-brain-starter/SKILL.md
```
...
### Step 3: Verify AGENTS.md routing
Check that the user's AGENTS.md has routing entries for all installed skills:
...
```markdown
# [skill name]
- **[skill-name]** (`~/.agents/skills/[skill-name]/SKILL.md`) — [description]. Trigger: `/[command]`
...
Check if the UserPromptSubmit hook is installed in `.codex/hooks.json`:

```bash
cat .codex/hooks.json | grep -q "UserPromptSubmit" && echo "Hook installed" || echo "MISSING"
```
