...
description: Starter recipe for the `skillOverrides` setting in `~/.codex/hooks.json`. Sharper auto-routing without disabling skills you might still need.
---

# Recommended skill overrides

Codex 2.1.129 added a `skillOverrides` setting that lets you control how skills participate in auto-routing. Three modes per skill:
...
Open `~/.codex/hooks.json` and add a `skillOverrides` block at the top level (between `model` and `statusLine` if those keys exist):
...
```bash
python3 -c "import json,os; json.load(open(os.path.expanduser('~/.codex/hooks.json')))"
```

Restart any open Codex session so it picks up the new settings.
...
- Codex 2.1.129 release notes (`skillOverrides` addition): https://github.com/anthropics/claude-code/releases
