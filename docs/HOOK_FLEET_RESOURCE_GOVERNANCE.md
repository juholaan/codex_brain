...
This repo's `hooks.json` is the source of truth for the Codex SessionStart
...
A SessionStart hook fires once per Codex session. A machine running many
...
```
bootstrap.sh
   └─ git clone (idempotent) ai-brain-starter@main → ~/plugins/codex-brain-starter/
        └─ scripts/install-hooks-user-level.py
              └─ reads the canonical root hooks.json
                    └─ merges its SessionStart block into ~/.codex/hooks.json
```
...
- **Native desktop / commercial products that wrap this substrate do not vendor
  or re-wire this fleet.** Where they spawn the Codex CLI for their own agent
...
2. **The protective reaper ships on SessionStart.** `remediate-runaway-procs.py`
   (CLASS-2 stuck-hook reaper) is the backstop that kills a wedged `~/.codex/hooks`
...
| `surface-orphan-codex-branches.py` | `git for-each-ref` + one `iterdir` | bounded git op + 1 dir level |
...
| `remediate-runaway-procs.py` | scan `~/.codex/hooks` procs, reap wedged ones | dual age/CPU gate, no FS walk |

None walks the user's vault corpus or `~/.codex/projects` transcripts.
...
- **per-event / per-tool substrate cold-start fan-out** vs `footprint-budgets.json`
  ...
  per-tool; `[ -f ~/.codex/hooks/… ]`-guarded maintainer hooks no-op on a fresh
...
python3 - "$FRESH/.codex/hooks.json" <<'PY'
import json, sys
