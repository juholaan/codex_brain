...
in each install's `~/.codex/hooks.json`, which:
...
- The command points at `~/.codex/hooks/X.py` or an absolute path, not the substrate
...
ADR 0005 named this gap explicitly and put it out of scope: *"Founder-personal `~/.codex`
...
`~/.codex/hooks.json` (override with `--settings`).

1. **Execute the LITERAL wired command, not a resolved repo file.** Each hook is run the
   way Codex runs it: the command string via `/bin/bash -c`, the event payload on
