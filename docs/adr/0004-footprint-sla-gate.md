...
1. **Hooks for one event run concurrently in Codex**, so felt wall-clock latency is `MAX(hook)`, not `SUM(hook)`. The summed "~1.5 s per turn" figure was wrong; the real per-event felt latency is the single slowest hook (tens of ms). The fan-out still costs real CPU/battery (N cold spawns) and reliability (one slow hook stalls the whole event to its timeout), but it is **not a wall-clock-latency emergency**.
