...
Per the Codex hooks doc, verbatim: *"`once` - If true, runs once per session
...
`~/.codex/hooks.json` - exactly where `once` is dead. So both hooks fired on
...
A hook author cannot set `cache_control` - the Codex harness owns the API
...
- Founder-personal `~/.codex` UPS injectors (e.g. a CONTEXT.md auto-loader) exhibit
