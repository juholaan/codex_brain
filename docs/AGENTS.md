...
- `session`. A worktree-scoped record of one Codex session. Drives `Last Session.md` rebuilds via `aggregate-sessions.py`.

## Reading the memory

Two surfaces today.

**graph-query MCP.** Codex-Code-native.

- Lives in vault `.codex/config.toml`.
...
Use this from any Codex session for targeted graph lookups in place of reading a 500KB graph report into context.
...
Honest framing: a stub. Multi-runtime consumption (any agent, not just Codex) is the explicit roadmap. Multi-runtime execution is not.
...
- Bring your own agent: Codex, the Anthropic SDK, OpenAI, Gemini, a custom runtime, anything that speaks MCP or HTTP.
...
1. **Read the memory.**
    - Install the `graph-query` MCP via vault `.codex/config.toml`, or
    - Run [`services/memory-api/`](../services/memory-api/) on localhost: `uvicorn app:app --host 127.0.0.1 --port 8765`.
    - The MCP path gives you typed tool calls inside Codex.
...
- The `graph-query` MCP shipped and stable across many Codex sessions.
...
- HTTP API and OpenAPI spec live and consumed by at least one non-Codex runtime.
