...
Build a Tool Routing section for their AGENTS.md based on what they use. Include ALL their tools, plus defaults for gaps:
...
**Rule:** When a task is better suited to another tool, say: "This is a [Tool] task — do it there, paste the result here if you need me to process it." Don't burn Codex tokens when another tool is faster.
```
...
"Do you have conversations saved in ChatGPT, Codex, Gemini, or any other AI tool? Those are some of your most valuable notes — they contain your thinking, your decisions, your brainstorming, your questions. Most people don't realize how much context is buried in their AI chat history."
...
- **Another assistant:** Use that product's account data export. Give me the resulting zip and I will convert supported conversation files.
...
The starter ships six richer templates in `~/plugins/codex-brain-starter/templates/obsidian/` — they use Templater syntax (date auto-fill, suggesters) so they need the Templater plugin installed (already covered in Phase 2-3). Copy any you want into your vault's `Meta/Templates/` folder:
...
cp ~/plugins/codex-brain-starter/templates/obsidian/*.md "/path/to/your/vault/Meta/Templates/"
```
...
Run a quick check on every skill folder. All installs live in bootstrap: if anything below is missing, re-run `bash ~/plugins/codex-brain-starter/bootstrap.sh` (Mac/Linux) or `powershell -NoProfile -ExecutionPolicy Bypass -File "$env:USERPROFILE\plugins\codex-brain-starter\bootstrap.ps1"` (Windows).
- `ls ~/.agents/skills/graphify/SKILL.md`
- `ls ~/.agents/skills/meeting-todos/SKILL.md`
- `ls ~/.agents/skills/patterns/SKILL.md`
- `ls ~/.agents/skills/insights/SKILL.md`
- `ls ~/.agents/skills/deconstruct/SKILL.md`
- `ls ~/.agents/skills/daily-journal/SKILL.md`
- `ls ~/.agents/skills/repurpose-talk/SKILL.md`
- `ls ~/.agents/skills/nano-banana/SKILL.md`
- `ls ~/.agents/skills/humanizer`
- `graphify --version`

Add skill routing to their AGENTS.md (global `~/.codex/AGENTS.md` if it exists, or vault root). Add ALL of these:
...
# daily journal
- **daily-journal** (`~/.agents/skills/daily-journal/SKILL.md`) — daily journal interview. Trigger: `$journal`
When the user types `$journal`, invoke the Skill tool with `skill: "daily-journal"` before doing anything else.

# humanizer
- **humanizer** (`~/.agents/skills/humanizer/SKILL.md`) — remove AI writing patterns from text. Trigger: `/humanizer`
...
- **insights** (`~/.agents/skills/insights/SKILL.md`) - journal pattern recognition. Triggers: `/weekly`, `/monthly`, `$insights`
...
- **patterns** (`~/.agents/skills/patterns/SKILL.md`) — extract recurring patterns from sessions. Trigger: `$patterns`
When the user types `$patterns`, invoke the Skill tool with `skill: "patterns"` before doing anything else.

# meeting todos
- **meeting-todos** (`~/.agents/skills/meeting-todos/SKILL.md`) — extract action items from a meeting note. Trigger: `$meeting-todos`
...
- **deconstruct** (`~/.agents/skills/deconstruct/SKILL.md`) — first-principles analyst. Trigger: `$deconstruct`
When the user types `$deconstruct`, invoke the Skill tool with `skill: "deconstruct"` before doing anything else.

# repurpose-talk
- **repurpose-talk** (`~/.agents/skills/repurpose-talk/SKILL.md`) - turn a speaking engagement into content pieces. Trigger: `$repurpose-talk`
When the user types `$repurpose-talk`, invoke the Skill tool with `skill: "repurpose-talk"` before doing anything else.

# nano-banana (image generation)
- **nano-banana** (`~/.agents/skills/nano-banana/SKILL.md`) — generate and edit images via Gemini. Trigger: `/nano-banana`
```
