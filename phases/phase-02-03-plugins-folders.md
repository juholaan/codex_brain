...
Tell the user: *"I'm going to install your Obsidian plugins in the background. These power live queries, templates, and task tracking, and let Codex work with your vault directly. Give me a few seconds."*
...
4. **Local REST API** — "Search 'Local REST API' → Install → Enable. Lets Codex interact with your vault directly."
...
**BEFORE CREATING ANYTHING — check what already exists.** The user may have already organized their vault. Scan the top-level folder first. If a folder already exists (even with a slightly different name), use it — don't create a duplicate. If a file was manually moved since setup, respect its new location. Codex's idea of "where something should go" is always subordinate to where it actually is right now. This prevents the most common setup complaint: Codex recreating files the user already moved.
...
**Why this matters:** previously, Writing/ was created by default for almost everyone because the conditional was too weak. The result was vaults with empty Writing folders for users who don't write, and Codex trying to create drafts in folders that shouldn't exist. Fix: require explicit opt-in.
...
if [ ! -f "[VAULT_PATH]/🏠 Home/About Me.md" ]; then
  cp "$HOME/plugins/codex-brain-starter/templates/Home/About Me.md" "[VAULT_PATH]/🏠 Home/About Me.md"
...
This is the canonical "deep profile" file. The AGENTS.md `## Me` line is the quick reference loaded every session; `About Me.md` is the rich profile loaded on demand for context-heavy work (panel reviews, life-history prose, advisory sessions, weekly insights).
