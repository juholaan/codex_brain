...
**MANDATORY ASK — never skip, regardless of what the user mentioned earlier.** If the user listed Gmail / Google Calendar / Outlook / Slack / any other tool back in Phase 4 question 3, that's INFORMATION; this phase is where Codex actually wires the MCP. The known failure mode: model captures "Gmail" in AGENTS.md `## Tools I Use` and then SKIPS this phase thinking the question is already answered. Phase 11 must fire even when prior phases already surfaced the tool name. The question changes from "do you use it?" to "you mentioned Gmail earlier — installing google-workspace-mcp now."

"Let's connect Codex to the tools you actually use. This is where the vault becomes an operating system, not just a notebook."
...
1. Clone: `git clone https://github.com/adelaidasofia/google-workspace-mcp ~/.codex/google-workspace-mcp`
2. Install deps: `cd ~/.codex/google-workspace-mcp && pip install -r requirements.txt`
...
4. Register in `~/.codex/config.toml` mcpServers block:
   ```json
   "google-workspace": {
     "command": "python3",
     "args": ["/Users/<user>/.codex/google-workspace-mcp/server.py"]
   }
   ```
...
6. Verify: restart Codex, check Settings → MCP, confirm `google-workspace` is listed with 61 tools across Gmail/Calendar/Drive/Docs/Sheets.
...
- **Discovery rule for the meeting workflow AGENTS.md section:**
...
- **Tell them:** "Gemini auto-creates a Google Doc transcript named like 'Meeting with [Name] - YYYY/MM/DD - Transcript' and saves it to a folder called 'Meet Recordings' in your Drive. Make sure that folder is shared/accessible from Codex. If you have the Google Drive MCP installed, you're set."
...
- Walk through each one separately. The AGENTS.md meeting workflow rule will list ALL discovery sources and tell Codex to **search them all in parallel** before reading anything. When multiple sources exist for the same meeting (e.g. Granola summary + verbatim Gemini transcript), the rule should prefer the verbatim source — see the "Source hierarchy" step below.
...
A short pointer in AGENTS.md is added separately (see the routing block below) — do NOT duplicate the rule body into AGENTS.md, that would force the model to reconcile two near-identical bodies and drift over time as one or the other is edited.
...
The skill itself was already installed in Phase 0 at `~/.agents/skills/meeting-todos/`. If it's missing, re-run the bootstrap (`bash ~/plugins/codex-brain-starter/bootstrap.sh` or `.ps1` on Windows).

Add routing to the user's AGENTS.md:

```markdown
# meeting todos
- **meeting-todos** (`~/.agents/skills/meeting-todos/SKILL.md`) — extract action items from a meeting note and add them to to-do. Trigger: `$meeting-todos`
