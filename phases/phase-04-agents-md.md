...
## Phase 4: Build Their AGENTS.md (and populate About Me)
...
**Write-through invariant.** Phase 4 produces TWO files at once: `AGENTS.md` (compressed quick reference loaded every session) and `🏠 Home/About Me.md` (the deep profile created in Phase 3b). Every answer the user gives — and every piece of personal context they volunteer beyond the question — must land in the right section of About Me as you go. Do NOT collect all 6 answers verbally and then write only to AGENTS.md. The two files fill together, in real time, as the user answers.

| Phase 4 question | Lands in AGENTS.md | Lands in About Me |
|---|---|---|
...
**Universal capture during Phase 4 (per Phase 3c):** when the user volunteers something while answering ANY of the 6 questions that doesn't fit the structured slot above, append it to the right About Me section. Example: question 2 is "key people." User says "Diego is my brother-in-law, we worked together at my last company before it sold in 2019." → AGENTS.md gets "Diego — brother-in-law"; About Me Relationships gets "Diego — brother-in-law"; About Me Work gets "Sold a previous company with Diego in 2019" under Past work. THREE distinct facts, three places.
...
**For question 2, also write a per-person CRM file** at `[VAULT_PATH]/👤 CRM/<First Last>.md` for every person the user names. The AGENTS.md `## People` section is the quick-reference index; the CRM file is the durable record that every future skill (meeting-todos, daily-journal, $patterns, panel attribution guard, advisory-panel exclusion) can read. Schema:
...
Now create the AGENTS.md at the vault root with this structure:
...
- 📝 Notes/
- ⚙️ Meta/
...etc. Do NOT leave this as a placeholder. A blank vault map means every future session lacks orientation and Codex will create duplicate folders.]
...
4. **Humanize external-facing prose before it leaves your hands.** For client emails, public posts, newsletters, essays, and other prose with a human audience, do a dedicated edit for natural voice and remove generic AI phrasing. If a reviewed `$humanizer` skill is installed, use it; otherwise perform the edit directly. **Scope:** prose only. Skip YAML, code, tables, dashboards, runbooks, meta files, journal entries, and single-line edits. For non-trivial changes, re-check the section you touched rather than rewriting the whole document.
...
**A. APPEND concise trigger pointers** to the user's AGENTS.md (NOT the full protocols — just the pointers). These tell Codex when to load the full protocol from the modular rule files:
...
for rule in ~/plugins/codex-brain-starter/templates/rules/*.md; do
...
(On Windows: `Copy-Item "$env:USERPROFILE\plugins\codex-brain-starter\templates\rules\session-start-checks.md" "[VAULT_PATH]\⚙️ Meta\rules\session-start-checks.md"` and same for the second and third files.)

Replace `{{DATE}}` in both files with today's date (YYYY-MM-DD).

**Why modular files instead of inlining into AGENTS.md:** The old approach appended ~320 lines of protocol detail directly into AGENTS.md, pushing it past the 10K-token read limit. Codex then needed multiple chunked reads at session start. The modular approach keeps AGENTS.md under 10K tokens (always loadable in one read) and puts the detailed protocols in standalone files that only get read when their trigger fires. Nothing is lost — the full protocols are right there in `⚙️ Meta/rules/`, and the pointers in AGENTS.md are strong enough that Codex can't skip them.
...
Tell them: "Your memory file is created. From now on, every Codex session in this vault starts with full context about who you are."

**STOP — verify before continuing.** Open the AGENTS.md you just created and confirm the `## Vault Map` section contains the actual folder list, not the placeholder text. If it's still a placeholder, fill it in now with the real folders from Phase 3. This is the most common setup failure — a blank vault map means Codex will create duplicate folders in every future session.
