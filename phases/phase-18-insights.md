...
Then create the skill file at `~/.agents/skills/insights/SKILL.md`:

```
**Insights skill template:** Read the full template from `templates/generated/insights-skill-template.md` and save it to `~/.agents/skills/insights/SKILL.md`. Replace `[VAULT_PATH]` with the user's actual vault path.
```

Then add routing to the user's AGENTS.md so `/weekly` and `/monthly` work as slash commands:
...
if [ -z "$CODEX_BIN" ]; then
  echo "$(date): ERROR — Codex CLI not found" >> "$LOG_FILE"
  exit 1
fi
...
"$CODEX_BIN" exec --full-auto \
  "Use the insights skill for a $PERIOD report. Read ~/.agents/skills/insights/SKILL.md and follow it exactly. Read all journal entries for the $PERIOD calendar period and save the full report to the correct folder. Then use the patterns skill in auto mode: read ~/.agents/skills/patterns/SKILL.md, scan for patterns, and automatically capture findings without asking for confirmation because this is a headless scheduled run. Save captures as concept notes, AGENTS.md rules, or writing seeds, wherever they fit best." \
  >> "$LOG_FILE" 2>&1
...
```powershell
# run-insights.ps1 - Generate weekly or monthly journal insight reports via Codex CLI
...
# Find Codex CLI (Windows)
$CodexBin = Get-Command codex -ErrorAction SilentlyContinue

if (-not $CodexBin) {
  Add-Content $LogFile "$(Get-Date): ERROR - Codex CLI not found"
  exit 1
}
...
& $CodexBin.Source exec --full-auto `
  "Use the insights skill for a $Period report. Read ~/.agents/skills/insights/SKILL.md and follow it exactly. Read all journal entries for the $Period calendar period and save the full report to the correct folder. Then use the patterns skill in auto mode: read ~/.agents/skills/patterns/SKILL.md, scan for patterns, and automatically capture findings without asking for confirmation because this is a headless scheduled run. Save captures as concept notes, AGENTS.md rules, or writing seeds, wherever they fit best." `
