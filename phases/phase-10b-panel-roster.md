...
If you find any:
1. Read the user's to-do file (check AGENTS.md for path — typically `Home/✅ Get to-do.md` or similar)
...
If Step 5 surfaced a **dissent** or an **omission flag**, automatically append it to a panel feedback log at `[VAULT_PATH]/Home/Panel Feedback Log.md` (or whichever path the user set up during Phase 4 for this log — check AGENTS.md). If the file doesn't exist, create it with a short header explaining it's a cross-context log of every real and synthetic panel reaction.
...
### Add $journal routing to AGENTS.md

After creating the journal skill, also add this block to the user's AGENTS.md so `$journal` works as a slash command:
...
Use whichever scheduling system is available in this Codex install. Try them in this order:
