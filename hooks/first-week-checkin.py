...
day 7, and day 14 injects a one-paragraph "how's it going?" prompt that:
  ...
  - Easy opt-out: `firstWeekCheckin: false` in AGENTS.md frontmatter
    OR delete ~/.codex/.ai-brain-checkin-state.json

Install detection:
  - Looks for ~/plugins/codex-brain-starter/.git for first-clone date
    OR ~/.codex/.ai-brain-installed-at marker (preferred)
  - If no marker AND no git repo, hook is a no-op

State file (~/.codex/.ai-brain-checkin-state.json):
  {
...
HOME = Path.home()
STATE_FILE = HOME / ".codex" / ".ai-brain-checkin-state.json"
INSTALL_MARKER = HOME / ".codex" / ".ai-brain-installed-at"
SKILL_DIR = HOME / ".codex" / "skills" / "ai-brain-starter"
...
def used_skills() -> set[str]:
    ...
    log_file = HOME / ".codex" / "logs" / "skill-usage.jsonl"
    skills: set[str] = set()
...
def build_checkin_message(milestone: str, days: int) -> str:
    ...
    gaps_note = ""
    gaps_file = Path.home() / ".codex" / ".ai-brain-starter-install-gaps.jsonl"
    ...
    return (
        ...
        f"(read from AGENTS.md if known, default English). Keep it warm and short, NOT a wall of text:\n\n"
        f"{config['body'].strip()}\n\n"
        f"{sug_text}\n\n"
