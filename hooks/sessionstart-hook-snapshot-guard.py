...
Behavior:
  - Snapshot SessionStart hook IDENTITIES to
    ~/.codex/state/sessionstart-hooks-snapshot.json (versioned: {"v":2,...}).
...
def main() -> int:
    ...
    if missing:
        ...
        print("[sessionstart-hook-guard] If intentional: `python3 ~/.codex/hooks/sessionstart-hook-snapshot-guard.py --refresh`. If not: re-add the missing hook(s).")
