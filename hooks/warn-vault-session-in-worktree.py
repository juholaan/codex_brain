...
The bug (MYC-575): the Codex Desktop app's per-session "worktree" checkbox
creates a `.codex/worktrees/<slug>/` checkout. On a CODE repo that is cheap and
...
code-repo worktree (Desktop `.codex/worktrees/` on a ~/dev repo, or the
sibling-dir `codex-dev-worktree` `~/dev/<repo>-<slug>/`) never fires.
  - Channel A (payload cwd): cwd is under a `.codex/worktrees/` segment. This
  ...
  - Channel B (transcript marker): cwd is the main root; the worktree id is in
    transcript_path as `--codex-worktrees-<slug>` (Desktop SessionStart, when the
    marker is present).
  - Channel C (process-cwd ground truth, payload-INDEPENDENT, ZERO subprocess): the
    ...
    `os.getcwd()` (the resolved physical path) carries the `/.codex/worktrees/`
...
~/.codex/hooks/ via install.sh). Negative-control test: warn-vault-session-in-worktree.test.sh.
...
WORKTREE_SEGMENT = "/.codex/worktrees/"
# Codex encodes the session cwd into the ~/.codex/projects/<dir> name by
# replacing path separators (and the leading dot of `.codex`) with dashes, so a
...
PROJECTS_WORKTREE_MARKER = "--codex-worktrees-"
...
def _split_segment(path: str) -> tuple[str, Path]:
    ...
    return str(Path(main_root) / ".codex" / "worktrees" / slug), Path(main_root)
...
def _detect(payload_cwd: str, transcript: str, event: str) -> tuple[str, Path] | tuple[None, None]:
    ...
    if payload_cwd and transcript and PROJECTS_WORKTREE_MARKER in transcript:
        ...
        if slug:
            return str(Path(payload_cwd) / ".codex" / "worktrees" / slug), Path(payload_cwd)

    # Channel C -- the hook's OWN process cwd is inside a .codex/worktrees/ checkout.
...
def main() -> int:
    ...
    body = (
        ...
        f"  2. Relaunch the vault PLAIN: `cd {main_root} && codex`\n"
