...
Behavior contract:
  - Reads user prompt from stdin (Codex hook contract)
...
Usage in hooks.json:
  "UserPromptSubmit": [{"hooks": [{"type": "command",
    "command": "python3 ${PLUGIN_ROOT}/hooks/detect-closing-signal.py"
  }]}]

Or via absolute path during install:
  python3 ~/plugins/codex-brain-starter/hooks/detect-closing-signal.py
...
User config (AGENTS.md at the resolved vault root, all optional):
...
Output marker file:
  ~/.codex/.closing-signal-{session_id}.json
...
def read_hook_input() -> dict:
    """Read JSON from stdin (Codex hook contract)."""
    try:
        raw = sys.stdin.read()
        if not raw.strip():
            return {}
...
def load_user_custom_signals(vault_root: Path) -> list[str]:
    """Read user's AGENTS.md for closingSignals.custom: [...]."""
    codex_md = vault_root / "AGENTS.md"
...
def load_user_suppress_signals(vault_root: Path) -> list[str]:
    """Read user's AGENTS.md for closingSignals.suppress: [...].
...
def load_user_custom_only(vault_root: Path) -> bool:
    """Read user's AGENTS.md for closingSignals.customOnly: true|false.
...
def ambiguous_haiku_check(prompt: str) -> bool:
    ...
    try:
        ...
        log_debug("openai SDK not installed; hybrid detection unavailable")
        return False
    try:
...
def derive_worktree(cwd: Path) -> str:
    ...
    cwd_str = str(cwd)
    m = re.search(r"/\.codex/worktrees/([^/]+)", cwd_str)
    if m:
        return m.group(1)
    git_file = cwd / ".git"
    if git_file.is_file():
...
def active_session_goal(transcript_path: str | None) -> str | None:
    ...
    The condition lives only in Codex's in-memory session state — there is
...
SKIP this phase entirely unless the OPTIONAL companion skill is installed:
  ~/.agents/skills/team-broadcast/scripts/auto-send.py
...
the team should know about, broadcast via:
  ...
  python ~/.agents/skills/team-broadcast/scripts/auto-send.py \\
...
6-step audit before close:
  ...
  2. Path resolution: grep every `~/.agents/skills/...` path in docs/templates
...
def main() -> int:
    ...
    try:
        ...
        # Skip detection on Stop-hook-feedback prompts. Codex injects
        ...
        # uniquely identify Codex feedback re-injection. Same session
        ...
        # artifacts on its throwaway codex/<slug> branch. See
