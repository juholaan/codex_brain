...
scratch checkouts. Nothing in stock Codex removes them when a session
...
# machine-local dir under ~/.codex — never into `main_repo`.
...
def find_main_repo(cwd: Path | None = None) -> Path | None:
    ...
    1. CLAUDE_PROJECT_DIR env var (set by Codex) if it's a real dir.
       If that value itself sits inside `.../.codex/worktrees/<slug>/`
       (Codex commonly sets it to the session cwd, which IS the
       ...
       claude/<slug> branch.
    2. If cwd is inside `.../.codex/worktrees/<slug>/`, the part before
       `.claude/worktrees`.
...
def current_worktree(cwd: Path | None = None) -> tuple[Path, str] | None:
    """If cwd is inside `.../.codex/worktrees/<slug>/`, return (path, slug)."""
    cwd = (cwd or Path.cwd()).resolve()
    marker = "/" + WORKTREES_SEG + "/"
    s = str(cwd)
    if marker not in s:
...
def live_session_cwds(main_repo: Path, grace_min: int = 35) -> set[str] | None:
    """Resolved cwd paths of currently-LIVE Codex sessions, from the session lock.
