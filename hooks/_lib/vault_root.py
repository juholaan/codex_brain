...
Bug this fixes: a machine-wide VAULT_ROOT default (set once, e.g. in Codex
...
working inside a SEPARATE vault-shaped repo (its own AGENTS.md, its own
...
(a AGENTS.md that declares its own Session End/Close cascade + an existing
...
TWO RESOLVERS, TWO SURFACES (MYC-3529)
    resolve_vault_root(cwd, env)  — SESSION-scoped. "Which vault does THIS
        session's close cascade write to?" Keyed on a AGENTS.md that declares
...
_WORKTREE_MARKER = "/.codex/worktrees/"


def collapse_worktree(path: Path) -> Path:
    """Collapse a `<vault>/.codex/worktrees/<slug>/...` path to `<vault>`.

    A worktree checkout carries the full tree, including AGENTS.md — so an
...
def _declares_own_session_close_cascade(candidate: Path) -> bool:
    ...
    Both signals required:
      - a AGENTS.md documenting its OWN "Session End"/"Session Close" cascade
        (heading match, case-insensitive), AND
      - an existing Meta folder to write into.
    Either alone is too weak: a AGENTS.md can mention sessions without owning
    a cascade for this purpose (a default vault's own AGENTS.md may use a
    ...
    folder can exist without any AGENTS.md ever declaring it canonical.
    """
    claude_md = candidate / "AGENTS.md"
...
def vault_root_for(target: Path) -> Path | None:
    ...
    Worktree paths collapse first: `<vault>/.codex/worktrees/<slug>/...` is
