...
Behavior:
  - Detect: scans every `~/.codex/projects/**/*.jsonl` against the
  ...
  - Auto-scrub (opt-in via `VAULT_ROOT` env var): if a JSONL's parent
    ...
    `~/.codex/secret-detection-log.jsonl` (structured records, NEVER
    secret values).
  - Mid-session safety: a JSONL whose worktree IS active is SKIPPED with
    ...
    JSONL corrupts Codex's resume state.
...
def _active_worktree_slugs(vault_root: Path) -> set[str] | None:
    ...
    parent worktree is mid-session — a mid-session scrub corrupts Codex
    Code's resume state.
...
def _log_scrub(record: dict) -> None:
    """Append a structured record to ~/.codex/secret-detection-log.jsonl.
