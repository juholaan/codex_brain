...
Once a worktree looks clean to Codex's archive flow, the harness
...
the main vault path, not under .codex/worktrees/. Surface them at
...
1. Wire into ~/.codex/settings.json Stop hooks. Place it FIRST so it
   runs before any autoprep/reconcile/auto-commit hook that might make
   the worktree look clean:
     "Stop": [
       {"hooks": [{
         "type": "command",
         "command": "python3 ~/.codex/hooks/snapshot-pending-work-on-stop.py 2>/dev/null || echo '{\"continue\":true,\"suppressOutput\":true}'"
       }]}
     ]
...
Vault root resolution order:
  ...
  2. Walk up from cwd until we find .codex/worktrees/<slug>/
...
def main() -> int:
    ...
    slug = cwd_str.split("/.codex/worktrees/", 1)[1].split("/", 1)[0]
    if not slug:
        return 0
