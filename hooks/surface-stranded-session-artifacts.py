...
A worktree's own checkout sits on a throwaway `codex/<slug>` branch. Anything
...
Complements surface-orphan-codex-branches.py:
  - that hook : committed-but-unmerged commits on codex/* branches.
...
def find_vault_root(cwd: Path) -> Path | None:
    ...
    find_vault_root() in surface-orphan-codex-branches.py.
    """
    parts = cwd.parts
    if ".codex" in parts:
...
def main() -> int:
    ...
    worktrees_dir = vault / ".codex" / "worktrees"
    ...
    msg = (
        ...
        "surface-orphan-codex-branches.py (committed-but-unmerged half)."
    )
