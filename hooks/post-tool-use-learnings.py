...
def find_vault_root(cwd: Path) -> Path | None:
    ...
    If we are inside a Codex git worktree (path contains
    `.codex/worktrees/<slug>/`), reset to the main vault root before walking
    ...
    # Worktree detection: path matches `.../<vault>/.codex/worktrees/<slug>/...`
    parts = p.parts
    if ".codex" in parts:
        i = parts.index(".codex")
        if i + 1 < len(parts) and parts[i + 1] == "worktrees":
            # Main vault root is the parent of the `.codex` segment.
            if i > 0:
                p = Path(*parts[:i])
