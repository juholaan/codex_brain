...
mkdir -p "$TMP/vaultmain/.obsidian" "$TMP/vaultmain/.codex/worktrees/slug"
mkdir -p "$TMP/devrepo/.codex/worktrees/slug"   # NO .obsidian -> code repo
...
mkgitwt() { # $1=root  ($2=obsidian => create .obsidian)  -> creates <root>/.codex/worktrees/wt
  git init -q "$1"
  [ "${2:-}" = "obsidian" ] && mkdir -p "$1/.obsidian"
  git -C "$1" -c user.email=t@t -c user.name=t commit -q --allow-empty -m init
  git -C "$1" worktree add -q "$1/.codex/worktrees/wt" HEAD 2>/dev/null
  [ -e "$1/.codex/worktrees/wt/.git" ] || { echo "SETUP-FAIL: could not create git worktree at $1/.codex/worktrees/wt"; fail=1; }
}
...
assert_fires  "A1 vault worktree cwd"      "$(run "$TMP/vaultmain/.codex/worktrees/slug" "")"
assert_silent "A2 plain vault cwd"         "$(run "$TMP/vaultmain" "")"
assert_silent "A3 code-repo worktree cwd"  "$(run "$TMP/devrepo/.codex/worktrees/slug" "")"
assert_silent "A4 bypass set"              "$(run "$TMP/vaultmain/.codex/worktrees/slug" "VAULT_WORKTREE_WARN_BYPASS=1")"
...
assert_fires  "C8 vault: payload=main+no-marker, PROCESS in real git worktree" \
              "$(run "$TMP/gitvault" "" "" "$TMP/gitvault/.codex/worktrees/wt")"
assert_silent "C9 code-repo: same shape, no .obsidian"   \
              "$(run "$TMP/gitcode" "" "" "$TMP/gitcode/.codex/worktrees/wt")"
...
ded() { ( cd "$TMP/gitvault/.codex/worktrees/wt" && printf '{"cwd":"%s"}' "$TMP/gitvault" \
...
out_pre_skip="$( cd "$TMP/gitvault/.codex/worktrees/wt" && printf '{"cwd":"%s","hook_event_name":"PreToolUse"}' "$TMP/gitvault" \
...
out_pre_a="$( printf '{"cwd":"%s","hook_event_name":"PreToolUse"}' "$TMP/vaultmain/.codex/worktrees/slug" \
    | env VAULT_WORKTREE_WARN_NODEDUP=1 VAULT_WORKTREE_WARN_STATE_DIR="$TMP/state" python3 "$HOOK" )"
...
: > "$TMP/gitcalls"
out_wtcatch="$( cd "$TMP/gitvault/.codex/worktrees/wt" && printf '{"cwd":"%s","hook_event_name":"UserPromptSubmit"}' "$TMP/gitvault" \
    | env PATH="$TMP/shim:$PATH" VAULT_WORKTREE_WARN_NODEDUP=1 VAULT_WORKTREE_WARN_STATE_DIR="$TMP/state" python3 "$HOOK" )"
...
out_t1="$( printf '{"cwd":"%s","hook_event_name":"UserPromptSubmit"}' "$TMP/vaultmain/.codex/worktrees/slug" \
