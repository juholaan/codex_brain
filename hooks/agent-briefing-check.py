...
# AGENTS.md (see canonical rule)). The framework names 4 delegation anti-patterns;
...
# AGENTS.md (see canonical rule) Candidate 5). A subagent whose job is
...
def main() -> None:
    ...
    if SKILL_DIR_RELATIVE_RE.search(prompt):
        issues.append(
            ...
            "(audited 2026-05-25 at AGENTS.md (see canonical rule))."
        )
    ...
    if ROUTER_PERSONA_RE.search(prompt):
        issues.append(
            ...
            "(audited 2026-05-26 at AGENTS.md (see canonical rule))."
        )
