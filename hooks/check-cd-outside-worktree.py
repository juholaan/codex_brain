...
(`<repo>/.codex/worktrees/<slug>/...`), block any `cd` / `pushd` whose
...
currently points at — not the worktree's `codex/<slug>` branch. Observed
...
Detection:
  - cwd (from payload) matches `<main>/.codex/worktrees/<slug>` → worktree
  ...
  - Block if the resolved target is `<main>` itself OR under `<main>/` but NOT
    under `<main>/.codex/worktrees/` (staying anywhere inside the worktrees
...
WORKTREE_RE = re.compile(r"^(?P<main>.+?)/\.codex/worktrees/(?P<slug>[^/]+)(?:/|$)")
...
# helpers are LOCAL on purpose: a shared ~/.codex/hooks/_lib/ module would be
...
def main() -> int:
    ...
    slug = m.group("slug")
    worktrees_prefix = main_root + "/.codex/worktrees/"
