...
AGENTS.md frontmatter or `SKILL_USAGE_TELEMETRY=1` env var.

What it does:
  - Detects `/skill-name` invocations in user prompts (regex `^/[a-z][a-z0-9-]+`)
  - Appends one JSONL line per invocation to ~/.codex/logs/skill-usage.jsonl
...
Privacy:
  ...
  - Opt-in mechanisms (any one enables):
      * `cascadeTelemetry: true` in AGENTS.md frontmatter
      * `SKILL_USAGE_TELEMETRY=1` env var
      * `~/.codex/.telemetry-opt-in` file exists
  - Storage: local only (never sent over network)
  - Anonymization: session_id is SHA-256 truncated to 12 chars
  - Skill name is NOT hashed (it's a public identifier from your AGENTS.md)
  - Prompt length bucketed into ranges, never exact
  - Opt-out at any time: delete ~/.codex/.telemetry-opt-in OR remove
    `cascadeTelemetry: true` from AGENTS.md
  - Erase history: `rm ~/.codex/logs/skill-usage.jsonl`
...
def telemetry_enabled(cwd: Path) -> bool:
    if os.environ.get("SKILL_USAGE_TELEMETRY") == "1":
        return True
    if (Path.home() / ".codex" / ".telemetry-opt-in").is_file():
        return True
...
def main() -> int:
    ...
    # reporters, ~/.codex/logs for tooling that runs without vault context.
    log_targets = []
    log_targets.append(Path.home() / ".codex" / "logs" / "skill-usage.jsonl")
