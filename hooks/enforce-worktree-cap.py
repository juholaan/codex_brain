...
DESIGN: two layers, reclaim-then-ALLOW, never block.
  ...
  Layer A — per-session backstop (runs EVERY session, regardless of count):
    * DEAD-SESSION reap — a registered `codex/<slug>` scratch worktree whose
      owning session is no longer live (per `.codex/.session-lock.json`) is
    ...
    * ORPHAN-DIR sweep — dirs under `.codex/worktrees/` that git no longer
  ...
  Layer B — count cap (ceiling for many concurrent LIVE sessions):
    ...
    that. So when the count is over the cap we reclaim the OLDEST idle `codex/<slug>`
...
WIRING (SessionStart):
  "SessionStart": [
    {"hooks": [{
      "type": "command",
      "command": "python3 ${PLUGIN_ROOT}/hooks/enforce-worktree-cap.py 2>/dev/null || echo '{\"continue\":true,\"suppressOutput\":true}'"
    }]}
  ]
"""
...
def _per_session_backstop(main_repo: Path, cur_path: Path | None) -> tuple[list[str], dict[str, str]]:
    ...
    regardless of the total count. Returns (reaped_dead, swept_orphans).
      ...
      1. DEAD-SESSION worktrees: a registered `codex/<slug>` scratch worktree whose
      ...
      2. ORPHAN DIRS: dirs under `.codex/worktrees/` that git no longer registers
    ...
    try:
        ...
        if live is not None:  # None = liveness unknown → never reap on this basis
            for wt in [w for w in (list_worktrees(main_repo) or []) if is_scratch_worktree(w)]:
                ...
                if not _branch(wt).startswith("codex/"):
