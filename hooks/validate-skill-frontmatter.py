...
Skill-context detection (in order):
  1. CODEX_ACTIVE_SKILL env var. Value is the skill name (slug).
  2. CODEX_ACTIVE_SKILL_PATH env var. Value is the absolute path to SKILL.md.
  3. Most recent SKILL.md from session metadata (if available).
  4. Fall through: no active skill, validation skipped.

Hook contract (Codex PreToolUse):
...
Pass 2 behavior:
  - Fires on every tool call when CODEX_ACTIVE_SKILL is set
...
def find_schema() -> Path | None:
    ...
    candidates = [
        ...
        Path.home() / ".codex" / "skills" / "ai-brain-starter" / "templates" / "schemas" / "skill.json",
    ]
    for c in candidates:
        if c.is_file():
            return c
...
def find_skill_md_for_name(skill_name: str) -> Path | None:
    ...
    Search order:
      ...
      2. ~/.agents/skills/<name>/SKILL.md (for installed user skills).
      3. ~/plugins/codex-brain-starter/skills/<name>/SKILL.md (bundled).
    """
    if not skill_name:
        return None
    here = Path(__file__).resolve().parent
    candidates = [
        ...
        Path.home() / ".codex" / "skills" / "ai-brain-starter" / "skills" / skill_name / "SKILL.md",
    ]
    for c in candidates:
        if c.is_file():
            return c
...
def resolve_active_skill_md() -> Path | None:
    ...
    Detection order (first match wins):
      1. CODEX_ACTIVE_SKILL_PATH env var (absolute path to SKILL.md).
      2. CODEX_ACTIVE_SKILL env var (skill name; resolve to a path).
      3. None (no active skill, sandboxing pass is skipped).
    """
    explicit_path = os.environ.get("CODEX_ACTIVE_SKILL_PATH", "").strip()
    if explicit_path:
        p = Path(explicit_path)
        if p.is_file():
            return p
        log_debug(f"CODEX_ACTIVE_SKILL_PATH {explicit_path!r} not a file")

    skill_name = os.environ.get("CODEX_ACTIVE_SKILL", "").strip()
    if skill_name:
        resolved = find_skill_md_for_name(skill_name)
        if resolved is not None:
            return resolved
        log_debug(f"CODEX_ACTIVE_SKILL {skill_name!r} not resolvable")
