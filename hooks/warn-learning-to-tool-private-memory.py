...
memory store — Codex's per-project memory at
``~/.codex/projects/<key>/memory/`` whose ``memory`` dir is a REAL directory (not
...
- Any path that is not a ``~/.codex/projects/*/memory`` store.
...
def _tool_private_memory_root(path: Path) -> Path | None:
    """Return the ``.../.codex/projects/<key>/memory`` ancestor of ``path``, or
    None if ``path`` is not inside a Codex-Code project memory store. Matches on
    the literal component names so it works for real ~/.codex paths and tmp test
    trees alike."""
    for anc in [path, *path.parents]:
        try:
            if (
                ...
                and anc.parent.parent.parent.name == ".codex"
            ):
                return anc
...
def main() -> int:
    ...
    if memory_root is None:
        return 0  # not a Codex-Code project memory store
