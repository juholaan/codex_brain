...
Wire into ~/.codex/settings.json SessionEnd hooks so it runs every session close.
...
3. Wire into ~/.codex/settings.json BOTH SessionEnd AND Stop hooks. SessionEnd
   ...
   every assistant turn and closes that window:
     "SessionEnd": [
       {"matcher": "", "hooks": [{
         "type": "command",
         "command": "python3 ~/.codex/hooks/reconcile-worktree-shared.py 2>/dev/null || echo '{\"continue\":true,\"suppressOutput\":true}'"
       }]}
     ],
     "Stop": [
       {"hooks": [{
...
def worktree_root(cwd: Path) -> Path | None:
    """Return the worktree root (.../.codex/worktrees/<slug>/), or None."""
    parts = cwd.parts
    try:
        idx = parts.index(".claude")
