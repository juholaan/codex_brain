...
The always-loaded text layer — global ~/.codex/AGENTS.md + the project's
AGENTS.md + MEMORY.md + the project's CONTEXT.md — is paid in tokens on EVERY
...
(token-economics), MEMORY.md has its own cliff guard, but the AGENTS.md kernel
files themselves silently grow until a new session opens already-full.

This hook measures that layer each SessionStart and:
  • warns if a file exceeds its hard ceiling (default: global AGENTS.md > 40 KB),
...
the command happened to run in). `discover()` finds the project AGENTS.md /
...
GLOBAL_KIND = "global AGENTS.md"

HOME = Path.home()
BASELINE_PATH = HOME / ".codex" / ".context-budget-baseline.json"
LASTWARN_PATH = HOME / ".codex" / ".context-budget-last-warn"
...
def _memory_md(cwd: str) -> Path | None:
    """Locate MEMORY.md the way Codex loads it (sanitized-cwd projects dir),
    ...
    try:
        ...
        cands.append(HOME / ".codex/projects" / san / "memory/MEMORY.md")
        m = re.match(r"(.*)--codex-worktrees-[^/]+$", san)
        if m:
            cands.append(HOME / ".codex/projects" / m.group(1) / "memory/MEMORY.md")
    except Exception:
        pass
    try:
        cur = Path(cwd)
...
def _project_file(cwd: str, name: str) -> Path | None:
    """Find <name> (AGENTS.md / CONTEXT.md) at the project root: CODEX_PROJECT_DIR
...
def discover(cwd: str) -> list[dict]:
    ...
    add(HOME / ".codex" / "AGENTS.md", GLOBAL_KIND, GLOBAL_CEILING)
    add(_project_file(cwd, "AGENTS.md"), "project AGENTS.md", None)
    ...
    # de-dup by resolved path (global and project AGENTS.md can coincide)
    seen: set[str] = set()
    out: list[dict] = []
    for it in items:
        try:
...
def is_project_anchored(items: list[dict]) -> bool:
    ...
    AGENTS.md, so a baseline recorded there is a too-narrow floor that would
...
def accept_mode(cwd: str | None = None) -> int:
    ...
    if not is_project_anchored(items):
        ...
        print(
            ...
            f"no project AGENTS.md / MEMORY.md / CONTEXT.md). A baseline recorded "
...
def self_test() -> int:
    ...
    # 1. POSITIVE: a global AGENTS.md over the 40KB ceiling must fire.
    ...
    items = [mk(20000, GLOBAL_KIND, GLOBAL_CEILING),
             mk(20000, "project AGENTS.md", None)]
    ...
    if not is_project_anchored([mk(40000, GLOBAL_KIND, GLOBAL_CEILING),
                                mk(5000, "project AGENTS.md", None)]):
    ...
    base4_kinds = [GLOBAL_KIND, "project AGENTS.md", "MEMORY.md", "project CONTEXT.md"]
    ...
    items = [mk(30000, GLOBAL_KIND, GLOBAL_CEILING),
             mk(8000, "project AGENTS.md", None)]
    ev = evaluate(items, {"total": 50000, "kinds": [GLOBAL_KIND, "project AGENTS.md"]})
    ...
    #    live CODEX_PROJECT_DIR so the real vault files can't leak into the check.
    old_home, old_base = HOME, BASELINE_PATH
    old_proj = os.environ.pop("CODEX_PROJECT_DIR", None)
    try:
        with tempfile.TemporaryDirectory() as td:
            fake_global = Path(td) / ".codex"
            fake_global.mkdir(parents=True)
            (fake_global / "AGENTS.md").write_text("x" * 50000, encoding="utf-8")
            HOME = Path(td)
            BASELINE_PATH = Path(td) / ".codex" / ".context-budget-baseline.json"
            its = discover(td)
        ...
        if old_proj is not None:
            os.environ["CODEX_PROJECT_DIR"] = old_proj
    ...
    #    positive control: from a project cwd (has a AGENTS.md) it accepts and
    ...
    try:
        with tempfile.TemporaryDirectory() as td:
            tdp = Path(td)
            (tdp / ".codex").mkdir(parents=True)
            (tdp / ".codex" / "AGENTS.md").write_text("g" * 39751, encoding="utf-8")
            HOME = tdp
            BASELINE_PATH = tdp / ".codex" / ".context-budget-baseline.json"

            # NEGATIVE: a bare subdir with no project AGENTS.md/MEMORY/CONTEXT.
            bare = tdp / "elsewhere"
            bare.mkdir()
            ...
            # POSITIVE: a project cwd with a AGENTS.md -> accept + persist kinds.
            proj = tdp / "proj"
            proj.mkdir()
            (proj / "AGENTS.md").write_text("p" * 6000, encoding="utf-8")
            with contextlib.redirect_stdout(io.StringIO()), \
                    contextlib.redirect_stderr(io.StringIO()):
            ...
            saved = load_baseline()
            if not saved or "project AGENTS.md" not in (saved.get("kinds") or []):
