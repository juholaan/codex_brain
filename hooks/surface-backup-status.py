...
WIRING (SessionStart):
  {"hooks": [{"type": "command",
    "command": "python3 ${PLUGIN_ROOT}/hooks/surface-backup-status.py 2>/dev/null || echo '{\"continue\":true,\"suppressOutput\":true}'"
  }]}
"""
...
CONF_PATH = Path(os.environ.get(
    "VAULT_BACKUP_CONF", str(Path.home() / ".codex" / ".vault-backup.conf")
))
...
if os.name == "nt":
    _PS1 = Path.home() / ".codex" / "skills" / "ai-brain-starter" / "scripts" / "vault-backup.ps1"
    _BACKUP_PREFIX = f'powershell -ExecutionPolicy Bypass -File "{_PS1}"'
else:
    _BACKUP_PREFIX = "bash ~/plugins/codex-brain-starter/scripts/vault-backup.sh"
