#!/usr/bin/env python3
"""
Lint Codex settings + MCP config files for silent breakage.
...
block at the bottom of `~/.codex/settings.json` to grant a few launchctl perms,
...
def default_targets() -> list[tuple[Path, str]]:
    candidates = [
        (HOME / ".claude" / "settings.json", "~/.codex/settings.json"),
        (HOME / ".claude" / "settings.local.json", "~/.codex/settings.local.json"),
