...
phrase. Codex had only a one-time SessionStart bullet to remember it. Users
...
Rule file lookup order (first hit wins):
  ...
  3. ~/plugins/codex-brain-starter/templates/rules/meeting-workflow.md
...
Wire into settings.json under hooks.UserPromptSubmit:
    {
      "type": "command",
      "command": "python3 ~/plugins/codex-brain-starter/hooks/inject-meeting-workflow-on-trigger.py 2>/dev/null || echo '{\"continue\":true,\"suppressOutput\":true}'"
    }
"""
import json
import os
...
def find_rule_file(vault_root: Optional[str]) -> Optional[str]:
    ...
    template = os.path.expanduser(
        "~/plugins/codex-brain-starter/templates/rules/meeting-workflow.md"
    )
...
1. Find the transcript. Probe in PARALLEL — do not pick one and ignore
   the rest:
   - **Granola** (macOS): run
     `python3 ~/plugins/codex-brain-starter/scripts/granola_sync.py`
...
7. Update to-dos. Read the user's to-do file path from AGENTS.md (it
