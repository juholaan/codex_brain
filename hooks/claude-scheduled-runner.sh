#!/bin/bash
# claude-scheduled-runner.sh — headless Codex runner for launchd / cron
# Usage: claude-scheduled-runner.sh <task-name>
#
# Runs a Codex scheduled task in headless (-p) mode.
...
#   CODEX_BIN       — path to claude binary (default: searches PATH then common locations)
#   TASKS_DIR        — where scheduled-task SKILL.md files live
#                      (default: ~/.codex/scheduled-tasks)
...
: "${CLAUDE:?Cannot find claude binary. Set CODEX_BIN or add claude to PATH.}"

TASKS_DIR="${TASKS_DIR:-$HOME/.codex/scheduled-tasks}"
