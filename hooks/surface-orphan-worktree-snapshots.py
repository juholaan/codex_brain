...
Codex archives the worktree (silently, between sessions), the
snapshot stays. This hook tells the user what's recoverable.

Reports ONLY orphaned snapshots — i.e., snapshot dirs whose
corresponding <vault>/.codex/worktrees/<slug>/ no longer exists.
...
1. Wire into ~/.codex/settings.json SessionStart hooks:
     "SessionStart": [
       {"hooks": [{
         "type": "command",
         "command": "python3 ~/.codex/hooks/surface-orphan-worktree-snapshots.py 2>/dev/null || echo '{\"continue\":true,\"suppressOutput\":true}'"
       }]}
     ]
...
Vault root resolution order:
  1. CODEX_PROJECT_DIR env var (set by Codex)
  2. cwd itself if it contains .codex/worktrees/
  3. Walk up from cwd until we find .codex/worktrees/
...
def find_vault_root(cwd: Path) -> Path | None:
    ...
    1. CODEX_PROJECT_DIR env var if set + valid.
    2. cwd itself if it contains .codex/worktrees/.
    3. Walk up cwd looking for the parent that contains .codex/worktrees/.
    """
