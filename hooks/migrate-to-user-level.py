...
<project>/.codex/worktrees/<name>/. User-level hooks
(~/.codex/settings.json) fire universally.
...
Behavior:
  ...
  - Fires AT MOST ONCE PER VAULT (state file at ~/.codex/.abs-migration-state.json).
  - Easy opt-out: `migrationDeclined: true` in AGENTS.md frontmatter.
...
def main() -> int:
    ...
    if project_has and not user_has and cwd_key not in notified_for:
        ...
        msg = (
            ...
            "fail to fire when Codex runs inside a git worktree (your "
            ...
            "primary language (read from AGENTS.md if known, default English). "
            ...
            "`migrationDeclined: true` in their AGENTS.md frontmatter so this "
