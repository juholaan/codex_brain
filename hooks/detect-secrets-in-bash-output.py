...
The Codex hook framework can't retroactively redact what the model
...
2. Append-only audit log at ~/.codex/hooks/secret-detection-log.jsonl.
...
# ~/.codex/hooks or another cwd. Sibling _lib/ dir on PYTHONPATH.
...
def _is_self_secret_inspection(payload: dict) -> bool:
    ...
    known-secrets stores (~/dev/*/.env*, ~/.codex/*/admin.env, ~/.zsh_secrets).
