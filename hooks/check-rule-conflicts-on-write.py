...
def find_vault_root(file_path: str) -> str:
    """Walk up from the edited file to find the vault root (contains AGENTS.md)."""
    if not file_path:
        return ""
    cur = os.path.dirname(os.path.abspath(file_path))
    while cur and cur != "/":
        if os.path.exists(os.path.join(cur, "AGENTS.md")) and os.path.exists(os.path.join(cur, "⚙️ Meta")):
            return cur
