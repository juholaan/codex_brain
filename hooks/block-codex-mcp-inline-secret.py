#!/usr/bin/env python3
"""PreToolUse Bash hook: block `codex mcp add ... --env <NAME>=<value>` when
<value> looks like a secret.

Pattern this prevents: every Codex child spawn includes the full MCP
config as a `--mcp-config '<json>'` argv string. Any `ps aux` / `pgrep -fl`
/ `lsof` on the running `codex` process dumps the PAT in plaintext. Past
...
Block applies to `codex mcp add` only. Other Codex MCP subcommands pass.
...
def main() -> int:
    ...
    # Only flag `codex mcp add ...` commands
    if not re.search(r"\bcodex\s+mcp\s+add\b", cmd):
        return 0
    ...
    for match in env_arg_re.finditer(cmd):
        ...
        if SECRET_RE.search(value):
            print(
                "[block-codex-mcp-inline-secret] BLOCKED:\n"
                "  Inlining a secret-looking value via `--env NAME=...` puts it\n"
                "  into the MCP config at ~/.codex.json, which every Codex\n"
                "  child spawn could expose process arguments. Any process\n"
                "  listing of the running Codex process may reveal plaintext.\n"
                "  will dump the secret in plaintext.\n\n"
                "  Correct pattern (env-injection):\n"
                "    1. echo 'export NAME=value' >> ~/.zsh_secrets\n"
                "    2. codex mcp add <server> -- <command>\n"
                "       (NO --env block; the child inherits shell env)\n"
                "    3. Restart Codex to pick up the new env\n\n"
                "  Past incident: 2026-05-19 github-mcp PAT leaked via\n"
                "  pgrep -fl chrome-devtools-mcp (the Bash output included\n"
                "  the agent process's full MCP configuration arguments.\n\n"
                "  Bypass: MCP_INLINE_SECRET_BYPASS=1",
                file=sys.stderr,
            )
            return 2  # block
