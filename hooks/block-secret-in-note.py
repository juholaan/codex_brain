...
(AWS / GitHub / OpenAI / OpenAI / Stripe-secret / Slack / Heroku / HubSpot /
...
itself, AGENTS.md quoting the patterns, a note documenting AWS's example key).

WIRING (PreToolUse, matcher "Write|Edit|MultiEdit"):
  {"type": "command",
   "command": "python3 ${PLUGIN_ROOT}/hooks/block-secret-in-note.py 2>/dev/null || echo '{\"hookSpecificOutput\":{\"hookEventName\":\"PreToolUse\",\"permissionDecision\":\"allow\"}}'"}
"""
...
BLOCK_NAMES = {
    "openai-api-key", "openai-api-key", "hubspot-private-app-token",
