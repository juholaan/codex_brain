...
(`~/.codex/hooks/`) for many minutes at high CPU is stuck by construction —
...
killing one loses nothing. Reaped here only when it is ALL of:
  * its command line is under `~/.codex/hooks/` (path-scoped — never a user
...
WIRING (SessionStart):
  "SessionStart": [
    {"hooks": [{
      "type": "command",
      "command": "python3 ${PLUGIN_ROOT}/hooks/remediate-runaway-procs.py 2>/dev/null || echo '{\"continue\":true,\"suppressOutput\":true}'"
    }]}
  ]
"""
...
def _should_reap_hook(
    ...
    *,
    ...
    process is a stuck `~/.codex/hooks/` python process per the dual age-AND-cpu
...
def main() -> int:
    ...
    # saw them (live parent, comm "Python"). Path-scoped to ~/.codex/hooks/
    ...
    if hook_reaped:
        ...
        msgs.append(
            ...
            f"~/.codex/hooks process, idempotent so it re-runs next session). "
