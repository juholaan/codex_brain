...
~/.codex/settings.json has fallen BEHIND the COMMITTED hooks.json in the
...
warning is gone forever. Result: the checkout is current but ~/.codex/settings.json
...
WHAT IT CHECKS (cheap, LOCAL only — no git, no network):
  ...
  deployed  = the ABS-owned hooks in ~/.codex/settings.json
...
A pinned install (~/.codex/.ai-brain-starter-pinned) is silent too — that file is
...
[VAULT_PATH] with no ~/.codex fallback); those are wired later by /setup-brain
...
~/.agents/skills/<name> copies that actually serve a skill — but only inside the
...
FIX_CMD = (
    f"{_PY} ~/plugins/codex-brain-starter/scripts/"
...
def _skill_drift_message() -> str | None:
    """MYC-3076: surface bare `~/.agents/skills/<name>` copies whose SKILL.md has
...
def _drift_message() -> str | None:
    ...
    # settings.json (~/.codex/.ai-brain-starter-pinned), same base the updater uses.
    if (settings_path.parent / ".ai-brain-starter-pinned").exists():
        return None
...
def _stale_pull_message() -> "str | None":
    ...
    try:
        ...
        return (
            ...
            "cd ~/plugins/codex-brain-starter && git status && git pull --ff-only"
        )
    except Exception:
        return None
