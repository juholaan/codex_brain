...
# Some long-running Codex desktop processes leak pty file descriptors over
...
PCT=$(( USED * 100 / MAX ))
if [ "$PCT" -ge 75 ]; then
  echo "[pty-pressure] macOS pseudo-terminal pool at ${PCT}% (${USED}/${MAX} ptys). The Codex desktop app can leak pty file descriptors over multi-day runs; at 100% no new terminal or shell can spawn (forkpty: Device not configured). Remedy: quit and relaunch Codex, or close idle terminals to free ptys."
fi
exit 0
