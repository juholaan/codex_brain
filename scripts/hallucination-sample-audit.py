#!/usr/bin/env python3
"""hallucination-sample-audit.py — sampled ground-truth audit of Codex's recent outputs.
...
recent assistant turns, vault-greps the keywords, and asks a separate Codex
...
def _derive_sessions_dir(vault: Path) -> Path:
    """Codex projects dir = ~/.codex/projects/<sanitized vault path>/"""
...
EXTRACT_SYSTEM = """You extract verifiable factual claims from a Codex assistant turn.
...
def find_recent_sessions(n: int) -> list[Path]:
    """Most-recently-modified Codex transcript JSONLs (skip .bak)."""
...
def extract_assistant_text(jsonl_path: Path) -> list[dict]:
    ...
    Schema is defensive — Codex transcripts evolve. We look for any
...
def extract_claims(text: str, max_claims: int, model: str) -> list[dict]:
    """Ask Codex to pull 1-N verifiable factual claims from an assistant turn."""
    user = (
...
def classify_claims(
    ...
    """Send claims + their grep excerpts to Codex for verdict."""
    if not claims:
        return []
    payload_lines = []
    ...
    if len(verdicts) < len(claims):
        ...
        for i in range(len(verdicts), len(claims)):
            verdicts.append({
                ...
                "verdict": "parse_error",
                "reasoning": "Codex response did not include this claim",
            })
    return verdicts
...
def render_report(run: dict) -> str:
    ...
    lines.append("")
    lines.append(
        f"Random sample of factual claims from Codex's recent assistant "
