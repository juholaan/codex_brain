...
External measurement of Codex's hallucination rate. Self-introspection is
untrustworthy; the only honest signal is: did a hook catch it, did the user
correct it, did a vault grep disprove it.

Three substrates:
1. ~/.codex/hookify-blocks.log — fabrications CAUGHT before shipping
...
def _derive_memory_dir(vault: Path) -> Path:
    """Codex projects dir for a vault: ~/.codex/projects/<sanitized>/memory/

    Encoding via the shared _project_key resolver — single source of truth that
    mirrors Codex's exact key (every non [A-Za-z0-9-] char -> '-') with a
...
HOOKIFY_LOG = Path(os.path.expanduser("~/.codex/hookify-blocks.log"))
...
def render_report(window_days: int, hookify: dict, cfi: dict, mem: dict) -> str:
    ...
    lines.append("related: [[Critical Failure Inventory]], [[AGENTS]]")
    lines.append("---")
    lines.append("")
    lines.append("# Hallucination Watch")
    lines.append("")
    lines.append(
        f"External measure of Codex's fabrication rate. Window: last "
    ...
    lines.append(
        ...
        "session, have a separate Codex session verify each against vault. "
