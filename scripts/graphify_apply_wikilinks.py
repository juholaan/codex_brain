...
create a stub note. The stub is seeded by:
  ...
  2. Calling Codex API to synthesize a Context section from only that text
     — no hallucination, Codex only sees sentences actually written in vault
...
def synthesize_context(name: str, ntype: str, mentions: list[tuple[str, str]]) -> str:
    """
    Call Codex API to synthesize a ## Context section from real vault mentions.
    Input is only text pulled from the vault — Codex adds no outside knowledge.
...
Return only the bullet points, each starting with -"""
    ...
    try:
        ...
        print(f"  ⚠ Codex API call failed ({e}) — Context will be blank")
        return "-"
...
def create_stub(
    ...
    if is_person:
        # No cap — read every mention chronologically so Codex sees the full arc.
