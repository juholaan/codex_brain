...
# Seed a AGENTS.md with per-user closingSignals.* keys to exercise the
...
def run_detector(prompt: str, claude_md: str | None = None) -> dict:
    ...
    When claude_md is provided, it is written to a AGENTS.md at the temp vault
    ...
    and never reads the developer's real vault AGENTS.md.
    """
    with tempfile.TemporaryDirectory() as tmp:
        if claude_md is not None:
            (Path(tmp) / "AGENTS.md").write_text(claude_md, encoding="utf-8")
        ...
        env["VAULT_ROOT"] = tmp  # hermetic: read THIS tmp's AGENTS.md, not the dev box's
...
def main() -> int:
    ...
    # Unify default fixtures (no AGENTS.md) with config-dependent fixtures.
