...
DETECTOR=""
for candidate in \
  "$HOME/plugins/codex-brain-starter/hooks/detect-closing-signal.py" \
...
INSTALLER=""
for candidate in \
  "$HOME/plugins/codex-brain-starter/scripts/install-hooks-user-level.py" \
...
if [[ -z "$INSTALLER" ]]; then
  ...
  HOOKS_SOURCE=""
  for candidate in \
    "$HOME/plugins/codex-brain-starter/hooks.json" \
