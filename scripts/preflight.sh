...
declare -a HOSTS=(
  ...
  "https://registry.npmjs.org|npm registry (Codex)"
  "https://claude.ai|claude.ai (Codex sign-in)"
)
...
# ─── 4. Codex (the desktop app + CLI) ───────────────────────────────────
section "$(t "Codex" "Codex")"

CC_OK=0
if [[ "$OS_KIND" == "Darwin" && -d "/Applications/Codex.app" ]]; then
  green "$(t "Codex desktop app found at /Applications/Codex.app" "Codex (app de escritorio) encontrado en /Applications/Codex.app")"
  CC_OK=1
fi
if have claude; then
  CC_VER="$(claude --version 2>/dev/null | head -1 | awk '{print $1}')"
  green "$(t "Codex CLI on PATH: ${CC_VER:-installed}" "Codex CLI en PATH: ${CC_VER:-instalado}")"
  CC_OK=1
fi
if [[ $CC_OK -eq 0 ]]; then
  red "$(t \
    "Codex is not installed. Install from https://claude.ai/download then re-run this check." \
    "Codex no está instalado. Instalalo desde https://claude.ai/download y volvé a correr esta verificación.")"
else
  yellow "$(t \
    "Make sure you are signed in to Codex with a paid plan (Pro, Max, or Team) before pasting the install prompt." \
    "Asegurate de estar logueado en Codex con un plan pago (Pro, Max o Team) antes de pegar el prompt de instalación.")"
fi
...
if [[ $RED_COUNT -gt 0 ]]; then
  ...
  if [[ $JSON_MODE -eq 0 ]]; then
    printf "${C_GRN}%s${C_RST}\n\n" "$(t \
      "Pre-flight PASSED. You're ready — paste the install prompt into Codex." \
      "Verificación OK. Listo — pegá el prompt de instalación en Codex.")"
  fi
  exit 0
fi
