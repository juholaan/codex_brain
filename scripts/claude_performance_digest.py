#!/usr/bin/env python3
"""
Codex Performance Self-Improvement System: Weekly Digest

Reads Codex JSONL session data, computes effectiveness metrics,
...
TODO_FILE = VAULT_ROOT / "⚙️ Meta" / "Codex To-dos.md"
...
def generate_report(sessions_data, agents_data, days):
    ...
    for rule, count in hookify_counts.most_common():
        if count >= THRESHOLDS["hookify_repeat"]:
            rx = f"Hookify rule '{rule}' fired {count} times. Escalate from reactive (hookify) to proactive (AGENTS.md rule or session-start reminder)."
            prescriptions.append(rx)
...
MEMORY_FILE = Path.home() / ".claude" / "AGENTS.md"


def _load_dedupe_sources(vault_root: Path) -> str:
    """Collect text from all places a shipped rule might already live.

    Checks: ~/.codex/AGENTS.md, vault AGENTS.md, cross-session MEMORY.md
    files under ~/.codex/projects/, vault rules files, and ~/.codex/hooks/*.py.
    Returns a single concatenated string for tag-presence checks.
    """
    chunks = []

    # 1. ~/.codex/AGENTS.md (global)
    global_claude = Path.home() / ".claude" / "AGENTS.md"
    ...
    # 2. Vault AGENTS.md
    vault_claude = vault_root / "AGENTS.md"
    ...
    # 3. Cross-session memory files under ~/.codex/projects/
    projects_root = Path.home() / ".claude" / "projects"
    ...
    # 5. ~/.codex/hooks/*.py (tags embedded in hook comments/code)
...
def apply_prescriptions(prescriptions, prescription_types):
    """Write behavioral rules to MEMORY.md, investigation items to Codex To-dos."""
    if not prescriptions:
        return 0, 0
    ...
    # ── Investigation items -> Codex To-dos ──
    existing_todos = ""
    if TODO_FILE.exists():
    ...
    if rules_written > 0:
        rule_summary = f"- [ ] **Performance: {rules_written} behavioral rule(s) written to AGENTS.md.** Review and validate: {', '.join(written_types)}. ({today}, {marker})"
...
def main():
    ...
    # Apply prescriptions: behavioral rules to MEMORY.md, investigations to Codex To-dos
    rules_written, todos_written = apply_prescriptions(prescriptions, prescription_types)
    if rules_written:
        print(f"Wrote {rules_written} behavioral rule(s) to AGENTS.md")
    if todos_written:
        print(f"Appended {todos_written} item(s) to Codex To-dos.md")
    if not prescriptions:
