#!/usr/bin/env python3
"""Make Codex's per-project memory physically live inside the vault.

THE PROBLEM THIS FIXES
----------------------
Codex writes its persistent memory (``MEMORY.md`` + topic files) to
``~/.codex/projects/<encoded-vault>/memory/`` — a per-account, per-machine,
...
``~/.codex/`` is a broken promise: it does not show up in the vault, it is not
...
2. Resolves the Codex project-memory path for this vault.
...
After this runs, everything Codex "remembers" is a file in the user's
...
def _link_dir(link: Path, target: Path) -> str:
    ...
    so failing there strands every future memory in ~/.codex, invisible in
    ...
    try:
        ...
        if proc.returncode != 0 or not _is_link(link):
            raise SystemExit(
                ...
                + "\n  Without this link your Codex memory stays in ~/.codex "
...
def _migrate_contents(src_dir: Path, dst_dir: Path, *, dry_run: bool, quiet: bool) -> int:
    ...
    for item in sorted(src_dir.rglob("*")):
        ...
        # the source is Codex's own memory, but be strict anyway).
...
def link_agent_memory(vault: str, *, dry_run: bool = False, quiet: bool = False) -> Path:
    ...
    _say("  Codex's memory now lives in your vault (visible in Obsidian, tracked by git).", quiet)
    return mem


def main() -> int:
    ap = argparse.ArgumentParser(description="Symlink Codex project memory into the vault.")
