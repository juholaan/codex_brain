...
Model routing (--llm-model):
  ...
  haiku    - Python template + Haiku via Codex
...
Discovers the health-mcp install at ~/.codex/health-mcp/ and imports its
...
def main() -> int:
    ...
    if not health_mcp_dir:
        print("ERROR: health-mcp install not found at ~/.codex/health-mcp/ or ~/dev/ai-brain-starter/services/health-mcp/. Run /health-setup first.", file=sys.stderr)
        return 2
