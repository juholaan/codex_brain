...
<#
  ...
  migrate Codex's path-keyed state (session transcripts + the agent-memory
  link) so prior session history survives the move.

  Windows parity of relocate-vault.sh (MYC-2383). Same contract, same Codex
  ...
  leave a link. A RAW move does the move but SILENTLY ORPHANS Codex
  history: Codex stores per-project state under <config>/projects/<key>, where
  ...
  Usage:
    ...
    relocate-vault.ps1 -MigrateCodexState <old-abs-path> <new-abs-path>
      (state-only: the vault is already at the new path; just fix Codex history)
  ...
  Options:
    ...
    -ConfigDir <dir>     Codex config dir (default: $env:CLAUDE_CONFIG_DIR
                         or ~/.codex)
    -Help                this help
...
# Codex's per-project dir name = the absolute cwd run through
...
# ---- migrate Codex path-keyed state (sessions + agent memory) -----------
function Move-CodexState { param([string]$OldAbs, [string]$NewAbs)
  $projdir = Join-Path $ConfigDir "projects"
  if (-not (Test-Path -LiteralPath $projdir)) {
    Say "  . no Codex projects dir ($projdir) - nothing to migrate"; return
  }
  ...
  if ($matched -eq 0) {
    Warn "  . no Codex session history found under old key '$oldkey*' in $projdir."
...
# state-only mode -------------------------------------------------------------
if ($MigrateCodexState) {
  if (-not $Old -or -not $New) {
    [Console]::Error.WriteLine("usage: relocate-vault.ps1 -MigrateCodexState <old-abs-path> <new-abs-path>"); exit 2
  }
  $OldA = "$(Get-AbsPath $Old)".Trim()
  $NewA = "$(Get-AbsPath $New)".Trim()
  Say "relocate-vault: migrating Codex state only ($OldA -> $NewA)"
  Move-CodexState -OldAbs $OldA -NewAbs $NewA
...
if (-not $Force) {
  ...
  if (Test-Path -LiteralPath $wt -PathType Container) {
    ...
    if ($active) { Die "an active Codex session is writing under .claude/worktrees (touched <10 min) - close it, or pass -Force" }
  }
}
...
if ($DryRun) {
  ...
  Move-CodexState -OldAbs $OldAbs -NewAbs $NewAbs
...
Say "relocate-vault: migrating Codex session history + agent memory ..."
Move-CodexState -OldAbs $OldAbs -NewAbs $NewAbs
...
if (-not $NoSymlink) { Say "  - Old path is a link:  $OldAbs -> $NewAbs   (scripts/hooks/AGENTS.md keep working)" }
Say "  - Reopen Obsidian; if it lost the vault, open $NewAbs"
Say "  - Reopen Codex in the vault - prior sessions appear in the picker"
