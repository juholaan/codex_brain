...
#                           wires a hook from ~/.codex/hooks/ that no route ever
...
INTEGRATION_TESTS=(
  ...
  # REAL ~/.codex/settings.json, pointing 95 of 111 hook entries at a temp
  ...
  # MCP secret-leak guards (MYC-3560): block-codex-mcp-inline-secret.py and
...
# The suite must never touch the developer's real ~/.codex/settings.json. It is
...
# deleting that worktree made every hook exit 2 (Codex's BLOCK signal) and
...
# the other 7 wrote elsewhere under ~/.codex, including SIX hook scripts copied
# straight into the live install at ~/plugins/codex-brain-starter/hooks/.
# A settings.json-only tripwire would have missed 70% of them. So it watches the
# paths a test can plausibly corrupt, and deliberately NOT the ones Codex
...
#
#   ~/.codex/hooks/  holds append-only logs (cwd-changed.log,
...
#   ~/.codex/state/  is not "the SessionStart snapshot". Measured: 93 files,
...
#
# DOTFILES, added 2026-08-13. ~/.codex/hooks/ is shared ground: this repo's
...
# ~/.codex/hooks/ — that is normal and is not the bug. The bug is the
...
if not fn:
    ...
    if 'name.startswith(".")' not in src:
        bad.append('the dotfile exclusion is gone from the walk — ~/.codex/'
...
# ~/.codex/hooks/ that nothing ever copies there. hooks.json guards those
