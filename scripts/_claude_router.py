"""Shared Codex-routing helper for vault scripts.

Routes Codex calls in priority order:
...
def _call_via_cli(
    ...
    SessionEnd hooks even in print mode, and a hook failing AFTER Codex wrote
...
def call_claude_text(
    ...
    """Call Codex and return the response text.
    ...
    True. The CLI path caches automatically — Codex handles it
...
def call_claude_json(
    ...
    """Call Codex and parse the response as JSON.
    ...
    try:
        ...
        raise ValueError(
            f"Codex returned non-JSON ({len(text)} chars): "
            f"{text[:300]!r}"
        ) from e
