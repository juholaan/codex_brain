...
Fire conditions (ALL must hold):
  ...
  2. Marker ~/.codex/.ai-brain-starter-email-on-file does NOT exist. The
...
Bypass:
  - touch ~/.codex/.ai-brain-starter-email-on-file   (settles it)
...
IF they give an email (use their name from AGENTS.md if you know it, else just the email):
```bash
...
SI dan un email (usá su nombre del AGENTS.md si lo sabés, si no, solo el email):
```bash
