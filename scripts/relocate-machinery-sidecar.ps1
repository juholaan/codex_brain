...
<#
  ...
  worktrees. So this REFUSES to run while any linked worktree or live Codex
...
function Test-NoLive {
  ...
  if ($wts.Count -gt 0 -or $sess -gt 0) {
    ...
    if ($sess -gt 0) { Err "  $sess live Codex session(s) in $VaultAbs/.claude/.session-lock.json" }
