...
once at startup. Answers targeted queries without requiring Codex to read the
...
Setup:
  1. Copy to ~/.codex/graph-query-mcp/server.py
  ...
  3. Register in .mcp.json:
       "graph-query": {
         "type": "stdio",
         "command": "fastmcp",
         "args": ["run", "~/.codex/graph-query-mcp/server.py"]
       }
  4. Restart Codex.
