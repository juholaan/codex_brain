...
#
# Designed to be called by Codex at the start of every session via a AGENTS.md
# rule. Outputs a structured, parseable result that Codex can translate into
# plain English for the user.
#
# Behavior:
#   - Runs at most once per day (tracked via ~/.codex/.ai-brain-starter-last-check).
...
#     entries (plain text), and exits 0. The CHANGELOG slice is what Codex
...
# by bootstrap.sh, which Codex calls if the user says "yes, install."
#
# Output format (parseable by Codex):
...
#   ---END---
#
# Usage (called from a Codex session via the AGENTS.md rule):
#   bash ~/plugins/codex-brain-starter/scripts/update-check.sh
#
# Force a re-check (bypass the once-per-day cooldown):
#   bash ~/plugins/codex-brain-starter/scripts/update-check.sh --force

set -uo pipefail

REPO_DIR="$HOME/plugins/codex-brain-starter"
CHECK_FILE="$HOME/.codex/.ai-brain-starter-last-check"
TODAY="$(date +%Y-%m-%d)"
FORCE=0
