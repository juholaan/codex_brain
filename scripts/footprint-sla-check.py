...
-------------------------------------------------------------------------
Hooks for one event run CONCURRENTLY in Codex, so felt WALL-CLOCK latency
...
Command classification - a wired command is one of:
  ...
  * userlevel-guarded: `[ -f ~/.codex/hooks/X.py ] && python3 ... || true` -
...
Modes:
  ...
  --measure-live [--execute] [--settings P] [--event E]
                        ...
                        ~/.codex/settings.json instead of the template: the per-message
...
# matches even the `[VAULT_PATH]/... || ~/.codex/...` fallback form because both
...
USERLEVEL_GUARD_RE = re.compile(r"\[\s*-f\s+[^\]]*~/\.codex/hooks/")
...
def _matcher_matches(matcher: str | None, tool: str) -> bool:
    ...
    counted directly, not per tool. A present matcher is a Codex tool-name
...
def event_fanout(fleet: dict, event: str) -> dict:
    ...
    ~/.codex/settings.json by install-hooks-user-level.py, and `once` is IGNORED
...
# in its ~/.codex/settings.json: the installer merges hooks.json INTO it, and it
...
# each wired injector EXACTLY as Codex does - the literal command string via
...
def _run_command_capture(command: str, payload: bytes, env_home: str, probe_cwd: str,
    ...
    Mirrors how Codex invokes a hook (shell command string + JSON stdin), so it
...
def cmd_measure_live(settings_path: Path, events: list[str], execute: bool,
    ...
    of an install's ACTUAL ~/.codex/settings.json (owned + unowned), with
    ...
    if not settings_path.exists():
        print(f"No settings file at {settings_path} - nothing to measure.\n"
              f"(--measure-live audits an INSTALL's live ~/.codex/settings.json, where "
...
def cmd_measure(hooks_json: Path, hooks_dir: Path, scripts_dir: Path,
    ...
    print("\nThis measured the SHIPPED hooks.json template. To measure your ACTUAL "
          "~/.codex/settings.json - including your OWN + customized injectors and "
...
_EVIL_HOOK = "python3 ~/plugins/codex-brain-starter/hooks/evil-{i}.py 2>/dev/null || echo '{{}}'"
...
def cmd_selftest() -> int:
    ...
    cases = [
        ("python3 ~/plugins/codex-brain-starter/hooks/x.py 2>/dev/null || echo '{}'",
         "substrate-python", "x.py"),
        ("python3 ~/plugins/codex-brain-starter/scripts/y.py 2>/dev/null || echo '{}'",
         "substrate-python", "y.py"),
        ("python3 '[VAULT_PATH]/plugins/codex-brain-starter/hooks/z.py' || "
         "python3 ~/plugins/codex-brain-starter/hooks/z.py || echo '{}'",
         "substrate-python", "z.py"),               # fallback form -> still substrate
        ("[ -f ~/.codex/hooks/retry-budget.py ] && python3 ~/.codex/hooks/retry-budget.py || true",
         "userlevel-guarded", None),
        ("bash '[VAULT_PATH]/⚙️ Meta/scripts/write-hook.sh'", "vault-script", None),
        ("if [ -f ~/.codex/.pinned ]; then echo '{}'; fi", "inline-bash", None),
    ]
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        ups_cmd = "python3 ~/plugins/codex-brain-starter/hooks/inj.py 2>/dev/null || echo '{}'"
        ...
        hj_inj.write_text(json.dumps({"hooks": {"UserPromptSubmit": [{"hooks": [
            {"type": "command", "command": "python3 ~/plugins/codex-brain-starter/hooks/uncond.py"},
            {"type": "command", "command": "python3 ~/plugins/codex-brain-starter/hooks/cond.py"},
        ]}]}}), encoding="utf-8")
        ...
        if not is_owned_fn("python3 ~/plugins/codex-brain-starter/hooks/log-skill-usage.py"):
            fails.append("AXIS-D-LIVE ownership: a substrate hook should tag as owned")
        if is_owned_fn("python3 ~/.codex/hooks/my-own-injector.py 2>/dev/null"):
...
def main() -> int:
    ...
    ap.add_argument("--measure-live", dest="measure_live", action="store_true",
                    help="(MYC-2396) advisory: per-message injected tokens of the LIVE "
                         "~/.codex/settings.json (owned + unowned + non-.py + per-tool)")
