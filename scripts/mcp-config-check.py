...
Catches the six silent-fail bugs most commonly encountered:
  ...
  4. Ghost ~/.codex/.mcp.json present (never honored by Codex)
  5. Orphan MCP directories with server.py but no registration
  6. Top-level mcpServers key in ~/.codex.json (ghost location, Codex ignores it;
     use projects[HOME_PATH].mcpServers via `codex mcp add -s user` instead)
...
def load_user_mcps() -> dict:
    """Return mcpServers from ~/.codex.json project-scoped entry for this vault.

    Codex honors user-scoped MCPs ONLY under projects[HOME_PATH].mcpServers,
...
def check_claude_json_top_level_ghost() -> list[tuple[str, str, str, str]]:
    """Flag top-level mcpServers in ~/.codex.json (ghost location, silently ignored)."""
    if not USER_CLAUDE_JSON.exists():
        return []
    try:
        with USER_CLAUDE_JSON.open("r", encoding="utf-8") as f:
    ...
    return [(
        ...
        f"Top-level mcpServers in ~/.codex.json is silently ignored by Codex. "
        f"Move {len(names)} server(s) ({', '.join(names)}) to "
        f"projects[HOME].mcpServers via `codex mcp add -s user` or delete them.",
    )]
...
def main() -> int:
    ...
    if GHOST_PATH.exists():
        issues.append((
            ...
            "Ghost MCP config exists at path Codex does not honor. Delete it.",
        ))
