...
rules completeness, and AGENTS.md sync status.
...
ROOT_CLAUDE_MD = VAULT_ROOT / "AGENTS.md"

# Memory directory for the vault project — auto-detect from vault path
# Codex encodes the vault path as the project directory name
# Codex encodes paths: / → -, spaces → -, leading dash kept
...
SIZE_CAPS: dict[str, tuple[Path, int]] = {
    "AGENTS.md (vault root)": (ROOT_CLAUDE_MD, 10_000),
    "\U0001f4d3 Journals/AGENTS.md": (
        VAULT_ROOT / "\U0001f4d3 Journals" / "AGENTS.md", 15_000
    ),
    "\u270d\ufe0f Writing/AGENTS.md": (
        VAULT_ROOT / "\u270d\ufe0f Writing" / "AGENTS.md", 8_000
    ),
...
def check_claude_md_sync(result: AuditResult) -> None:
    """Verify root AGENTS.md references all rules files."""
    if not ROOT_CLAUDE_MD.exists():
        result.warn("AGENTS.md sync", "root AGENTS.md does not exist")
        return
    ...
    if unreferenced:
        result.warn(
            "AGENTS.md sync",
            f"root AGENTS.md does not reference: {', '.join(unreferenced)}",
        )
    else:
        result.passed(
            "AGENTS.md sync",
            "root AGENTS.md references all rules files",
        )
