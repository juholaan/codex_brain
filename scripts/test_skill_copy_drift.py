...
(~/plugins/codex-brain-starter) self-updates on the ~6-day auto-update, but
the propagation of that skill CONTENT into the bare copies that actually serve a
skill (~/.agents/skills/<name>, plain dirs) runs ONLY inside the auto-update's
...
class ClassifyDriftTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.install = base / "install"          # ~/.agents/skills
