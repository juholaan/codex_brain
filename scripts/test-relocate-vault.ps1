...
<#
  ...
  Windows box; everything else (arg parse, move, Codex path-key migration,
...
try {
  ...
  Set-Content -LiteralPath (Join-Path $old1 "AGENTS.md") -Value "# brain"
  ...
  # ---- 2. REAL relocate: move + link + Codex-state migration + manifest ----
  $old2 = Join-Path $TMP "cloud2/Brain"; New-Item -ItemType Directory -Force -Path $old2 | Out-Null
  Set-Content -LiteralPath (Join-Path $old2 "AGENTS.md") -Value "# brain"
  ...
  Check "relocate: new is a real dir w/ notes" ((Test-Path -LiteralPath (Join-Path $new2 "AGENTS.md")))
  ...
  Set-Content -LiteralPath (Join-Path $old4 "AGENTS.md") -Value "# brain"
  ...
  $r = Run-RV -RvArgs @("-MigrateCodexState", $old5, $new5) -ConfigDir $cfg5
  ...
  Set-Content -LiteralPath (Join-Path $old6 "AGENTS.md") -Value "# brain"
  ...
  try {
    ...
    Check "obsidian gate: -Force moved the vault (new exists)"          (Test-Path -LiteralPath (Join-Path $new6 "AGENTS.md"))
  } finally {
    $env:RELOCATE_VAULT_OBSIDIAN = "absent"
  }
}
