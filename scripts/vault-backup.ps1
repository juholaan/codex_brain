...
<#
  ...
  Config:  ~/.codex/.vault-backup.conf  (JSON, keyed by resolved vault path - shared with check-vault-backup.py)
...
function Cmd-Verify {
  ...
  try {
    ...
    $sentinel = if (Test-Path -LiteralPath (Join-Path $tmp "AGENTS.md")) { ", AGENTS.md present" } else { "" }
