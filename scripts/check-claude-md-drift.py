#!/usr/bin/env python3
"""
check-claude-md-drift.py — detect stale entries in vault AGENTS.md.

Over months a AGENTS.md grows stale: people no longer in the user's life,
...
It does NOT auto-edit AGENTS.md. Drift is a judgment call — the script
...
Detection signals:
  ...
  3. Wikilinks in AGENTS.md that point to files that don't exist in the
     vault → broken link, candidate for repair.
  4. Identical headings appearing twice in AGENTS.md → likely accidental
...
Read-only. Never modifies AGENTS.md.
"""
...
def render_report(report: dict) -> str:
    ...
    if report["dormant_people"]:
        ...
        lines.append("")
        lines.append("These names appear in AGENTS.md `## People` but were not found in any session, decision, or journal entry in the window.")
        lines.append("")
    ...
    if report["broken_links"]:
        lines.append("## Broken wikilinks in AGENTS.md")
        lines.append("")
    ...
    if not any([report["dormant_people"], report["dormant_focus"], report["broken_links"], report["duplicate_headings"], report["old_codified"]]):
        lines.append("No drift signals detected. AGENTS.md looks current.")
        lines.append("")
    return "\n".join(lines)
...
def main() -> int:
    ...
    if not claude_md.is_file():
        print(f"No AGENTS.md at {claude_md}", file=sys.stderr)
        return 1
    ...
    if args.quiet:
        ...
        print(f"AGENTS.md drift: {signals} signal categor{'y' if signals == 1 else 'ies'} flagged.")
    else:
        print(out)
