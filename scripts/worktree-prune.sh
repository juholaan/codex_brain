...
# logic invoking it. ~/.codex/hooks/ is LAST and legacy-only: no installer ever
...
for _c in "$SCRIPT_DIR/worktree-reclaim.py" \
          "$HOME/plugins/codex-brain-starter/scripts/worktree-reclaim.py" \
          "$HOME/.codex/hooks/worktree-reclaim.py"; do
...
mkdir -p "$(dirname "$LOG")"
{
  ...
  while IFS= read -r branch; do
    slug="${branch#claude/}"
    [ -d "$VAULT_ROOT/.codex/worktrees/$slug" ] && continue
