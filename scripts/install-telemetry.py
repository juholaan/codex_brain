...
`~/.codex/.ai-brain-starter-install.jsonl` AND updates the progress
marker at `~/.codex/.ai-brain-starter-progress.json`. `summarize`
...
surfaced 5 Codex improvisations that the SKILL.md didn't anticipate.
...
def main(argv: list[str] | None = None) -> int:
    ...
    p_append.add_argument(
        "--new-improvisation",
        default=None,
        help="short description of an improvisation Codex did that the skill did not prescribe",
    )
    p_append.add_argument(
        "--redirected",
        action="store_true",
