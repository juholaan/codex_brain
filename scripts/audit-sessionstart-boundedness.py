...
A SessionStart hook fires once per Codex session. A machine running N
...
~/.codex/projects transcript tree, ...) without concurrency + wall-clock bounds
...
def resolve_command_path(cmd: str) -> Path | None:
    ...
    path. Handles `python3 ~/.codex/hooks/x.py --flag`, quoted paths, and a
...
def main():
    for p in Path('~/.codex/state/snapshots').expanduser().rglob('*'):
        pass
"""
_FX_NO_WALK = """#!/usr/bin/env python3
from pathlib import Path
def main():
    for p in Path('~/.codex/worktrees').expanduser().iterdir():
        pass
"""
...
find ~/.codex/state -maxdepth 1 -name '*.json'
"""
