...
# Codex's path-keyed state (session transcripts + the agent-memory symlink)
...
# symlink. A RAW `mv` does the move but SILENTLY ORPHANS Codex history:
# Codex stores per-project state under <config>/projects/<key>, where <key> is
...
# as a backup), so reopening Codex in the relocated vault shows the history.
#
# Pure bash + python3 (only for the key transform, to match Codex's
...
#   --config-dir <dir>   Codex config dir (default: $CLAUDE_CONFIG_DIR or ~/.codex)
#   -h, --help           this help
#
...
SWEEP_EXTRA=()
CONFIG_DIR="${CLAUDE_CONFIG_DIR:-$HOME/.codex}"
...
# abspath/pathkey via python3 so the key transform matches Codex's JS
...
# projkey: Codex's per-project dir name = the absolute cwd run through
...
migrate_claude_state() {  # $1=old-abs  $2=new-abs
  ...
  if [ ! -d "$projdir" ]; then
    say "  · no Codex projects dir ($projdir) — nothing to migrate"
    return 0
  fi
  local oldkey newkey
  oldkey="$(projkey "$old")"
...
# =============================================================================
# state-only mode: the vault is already at the new path; just fix Codex history
# =============================================================================
if [ "$MIGRATE_ONLY" = 1 ]; then
  ...
  OLD_ABS="$(abspath "$OLD")"
  NEW_ABS="$(abspath "$NEW")"
  say "relocate-vault: migrating Codex state only ($OLD_ABS -> $NEW_ABS)"
...
if [ "$FORCE" != 1 ]; then
  ...
  if [ -d "$OLD_ABS/.claude/worktrees" ]; then
    ...
    if [ -n "$active" ]; then
      die "an active Codex session is writing under .claude/worktrees (touched <10 min) — close it, or pass --force"
    fi
  fi
...
[ "$NOSYMLINK" = 1 ] || say "  - Old path is a symlink: $OLD_ABS -> $NEW_ABS   (scripts/hooks/AGENTS.md keep working)"
