...
# and concurrent Codex sessions all competing for .git/index.lock.
#
# Usage:
...
#
# Flags:
#   --kill-leaked   Kill leaked git-status/diff children of Codex.app
#                   before attempting the commit. Use when you know
#                   Codex.app is actively polling this vault.
#
...
# --- kill leaked Codex.app git children (optional, macOS) ---
if [ "${KILL_LEAKED}" = "1" ]; then
    CLAUDE_PID=$(pgrep -f "Codex.app/Contents/MacOS/Codex" 2>/dev/null | head -1 || true)
    if [ -n "${CLAUDE_PID}" ]; then
        pkill -P "${CLAUDE_PID}" -f "git " 2>/dev/null && \
            log "killed leaked git children of Codex.app PID ${CLAUDE_PID}" || \
