...
Codex from the *right* working directory. Codex loads a vault's AGENTS.md
by walking UP from the launch cwd to the filesystem root — so a vault launched
from the wrong folder loads only the global ~/.codex/AGENTS.md, answers
...
(diagnose.sh and context-audit.py already do existence). It simulates Codex
Code's AGENTS.md ancestor-walk and answers one deterministic question:

    "If Codex is launched from <launch-dir>, will the vault's personalized
     context load on the first run?"

It also emits the exact `cd "<vault>" && codex` command, so the install can
...
Verdicts (--porcelain prints the single most-severe token):
  OK_WILL_LOAD               personalized AGENTS.md will load from launch-dir
  FAIL_NO_CLAUDE_MD          no AGENTS.md at the vault root
  FAIL_TEMPLATE_UNFILLED:<d> AGENTS.md is the unfilled template or a stub
                             (d = "stub" or the placeholder count)
  FAIL_WRONG_CWD:<dir>       launch-dir is not inside the vault, so the vault
                             AGENTS.md is NOT on the cwd->root path (won't load)
  WARN_MISSING_CONTEXT:<n>   AGENTS.md references session-start files that are
...
# Minimum size below which a AGENTS.md is treated as a stub, not a real
...
# (templates/generated/agents-md-template.md) — a real personalized AGENTS.md
...
# Canonical session-start context files the substrate's AGENTS.md tells Codex
# to read on every session. Checked by BASENAME and only when AGENTS.md actually
# references them — so a vault whose AGENTS.md never mentions a file is never
...
def _is_within(child: Path, parent: Path) -> bool:
    ...
    cwd->root walk from `child`, which is exactly when Codex loads
    parent/AGENTS.md."""
...
def _ancestor_claude_mds(vault: Path) -> list[Path]:
    """AGENTS.md files in STRICT ancestors of the vault. If the user launches
    ...
    from the vault itself still loads the vault AGENTS.md."""
    found: list[Path] = []
...
def evaluate(vault: Path, launch_dir: Path) -> dict:
    ...
    checks: list[dict] = []
    claude_md = vault / "AGENTS.md"
    launch_command = 'cd "%s" && codex' % vault

    # --- Check 1: AGENTS.md exists at the vault root -------------------------
    if not claude_md.is_file():
        checks.append({"name": "AGENTS.md present", "status": "fail",
                       "detail": "no AGENTS.md at vault root %s" % vault})
    ...
    checks.append({"name": "AGENTS.md present", "status": "pass",
                   "detail": "%d bytes" % size})
    ...
    if size < MIN_PERSONALIZED_BYTES:
        checks.append({"name": "AGENTS.md personalized", "status": "fail",
    ...
    if placeholders:
        ...
        return {"verdict": "FAIL_TEMPLATE_UNFILLED:%d" % len(placeholders),
                ...
                "launch_command": launch_command, "checks": checks}
    checks.append({"name": "AGENTS.md personalized", "status": "pass",
    ...
    if not _is_within(launch_dir, vault):
        checks.append({"name": "Launch cwd loads the vault", "status": "fail",
                       ...
                       "AGENTS.md is not on its cwd->root path and will not load"
                       % launch_dir})
    ...
    # --- Soft: a AGENTS.md in an ancestor could shadow on a wrong-cwd launch -
    shadows = _ancestor_claude_mds(vault)
    if shadows:
        checks.append({"name": "Ancestor AGENTS.md", "status": "info",
                       "detail": "non-vault AGENTS.md above the vault: %s "
    ...
    if missing:
        checks.append({"name": "Context layer resolves", "status": "warn",
                       "detail": "AGENTS.md references but the vault is missing: "
                       + ", ".join(missing)})
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument("--launch-dir", default=None,
                        help="directory Codex would launch from "
