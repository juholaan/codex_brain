...
$InstallDir  = "$env:USERPROFILE\.agents\skills"
$CooldownFile = "$env:USERPROFILE\.codex\.ai-brain-starter-drift-check-last-run"
$IgnoreFile  = "$env:USERPROFILE\.codex\.ai-brain-starter-drift-check-ignore"
...
if ($Vault) {
    $VaultCodexMd = Join-Path $Vault "AGENTS.md"
    $RulesDir = Join-Path $StarterDir "templates\rules"
    if ((Test-Path -LiteralPath $VaultCodexMd) -and (Test-Path -LiteralPath $RulesDir)) {
        $vaultCodexContent = Get-Content -LiteralPath $VaultCodexMd -Encoding UTF8
        ...
        Get-ChildItem -LiteralPath $RulesDir -Filter "*.md" -File -ErrorAction SilentlyContinue | ForEach-Object {
            ...
            # Heading must appear verbatim in the user's AGENTS.md
            if (-not ($vaultCodexContent -contains $heading)) { return }
            ...
            $installedBlock = Get-MarkdownBlock -FilePath $VaultCodexMd -Heading $heading
            ...
            if ($installedNormalized -ne $templateNormalized) {
                Add-Drift -Scope "vault-rule" -InstalledPath $VaultCodexMd -RepoSourcePath $ruleFile -Note "block heading: $heading"
            }
        }
    }
}
