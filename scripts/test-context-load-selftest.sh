...
# self-test. It simulates Codex's AGENTS.md ancestor-walk so the install
...
# A filled, personalized AGENTS.md (no template placeholders). References the
...
# Create the canonical session-start context files the filled AGENTS.md points at.
write_context_layer() {
...
# --- Fixture 4: no AGENTS.md at vault root ----------------------------------
...
# --- Fixture 6: stub AGENTS.md (too small to be real personalization) -------
...
# --- Fixture 8: filled AGENTS.md but the context layer it references is gone -
...
if printf '%s' "$human_out" | grep -q "cd .*good.* && codex"; then
