...
V="$ROOT/Brain"
mkdir -p "$V/⚙️ Meta" "$V/.codex/worktrees/scratch" "$V/.smart-env" "$V/.codegraph"
printf '# AGENTS.md\nbrain memory\n' > "$V/AGENTS.md"
printf 'a private journal entry\n'   > "$V/journal.md"
printf 'EXHAUST-should-not-appear\n' > "$V/.codex/worktrees/scratch/junk.md"
...
printf '%s\n' "$listing" | grep -q './AGENTS.md'  && pass "AGENTS.md is in the archive" || fail "AGENTS.md missing from archive"
...
if command -v gpg >/dev/null 2>&1 || command -v openssl >/dev/null 2>&1; then
  ...
  mkdir -p "$V2"; printf '# AGENTS.md\n' > "$V2/AGENTS.md"; printf 'secret journal\n' > "$V2/secret.md"
  ...
  # on ~/.codex existing — the env-correlated flake fixed in MYC-1804.
