...
$hosts = @(
    ...
    @{ url = "https://registry.npmjs.org"; name = "npm registry (Codex)" }
    @{ url = "https://claude.ai"; name = "claude.ai (Codex sign-in)" }
)
foreach ($h in $hosts) {
...
$claudePaths = @(
    "$env:LOCALAPPDATA\Programs\claude\Codex.exe",
...
$claudeOk = $false
if ($claudeApp) {
    Green (T "Codex desktop app found at $claudeApp" "Codex (app de escritorio) encontrado en $claudeApp")
    $claudeOk = $true
}
if (Have claude) {
    $cv = (& claude --version 2>$null | Select-Object -First 1)
    Green (T "Codex CLI on PATH: $cv" "Codex CLI en PATH: $cv")
    $claudeOk = $true
}
if (-not $claudeOk) {
    Red (T "Codex is not installed. Install from https://claude.ai/download then re-run this check." `
            "Codex no está instalado. Instalalo desde https://claude.ai/download y volvé a correr esta verificación.")
} else {
    Yellow (T "Make sure you are signed in to Codex with a paid plan (Pro, Max, or Team) before pasting the install prompt." `
...
$absDir = "$env:USERPROFILE\plugins\codex-brain-starter"
...
if ($script:Red.Count -gt 0) {
    ...
    if (-not $Json) {
        Write-Host (T "Pre-flight PASSED. You're ready - paste the install prompt into Codex." `
                       "Verificación OK. Listo - pegá el prompt de instalación en Codex.") -ForegroundColor Green
        Write-Host ""
    }
    exit 0
}
