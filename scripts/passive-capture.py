...
Scans recent Codex session transcripts for user utterances that
...
matching canonical surface (AGENTS.md, rules/*.md, Decision Log), the
rest go to Passive Captures/Archive/.

Idempotent via state file at `~/.codex/passive-capture-state.json`:
...
State: `~/.codex/passive-capture-state.json`
...
def _resolve_transcripts_dir():
    """Locate the per-account Codex transcript dir for this vault.

    Pattern: ~/.codex/projects/<encoded-vault-path>/ where the encoded
    path replaces `/` and `.` with `-` and prepends `-`. Returns None if
    not found (Codex not installed, different account, etc.).
    """
...
def write_capture(capture: Capture, dry_run: bool = False) -> Path | None:
    ...
    body = [
        ...
        "",
        "Choose one of:",
        "- [ ] **Adopt as rule** → move to `AGENTS.md` or `⚙️ Meta/rules/<file>.md`. Mark this file `triage_status: adopted-rule`.",
