...
into the user's installed ~/.agents/skills/ directory.
...
surface it to Codex (who surfaces it to the user).
...
Usage: python3 ~/plugins/codex-brain-starter/scripts/sync-skills.py
...
# never re-fires, so the clone can sit AHEAD of the bare `~/.agents/skills/<name>`
...
def drift_message(drifts: list[dict]) -> str | None:
    ...
    fix = f"{py} ~/plugins/codex-brain-starter/scripts/sync-skills.py"
...
def main() -> int:
    ...
    # sync-skills syncs skill -> ~/.agents/skills; sync-vault-scripts.sh is the
...
installed ~/.agents/skills/ directory.
