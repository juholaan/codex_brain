...
#   2. graphify                   expensive LLM, skipped here (needs Codex)
...
# Invoke the /second-brain-mapping slash command from Codex to drive all three,
...
if [[ $WIKILINKS_ONLY -eq 0 ]]; then
  ...
  echo "  Skipped in bash (graphify is LLM-orchestrated, needs Codex)."
  echo "  To run: invoke /second-brain-mapping in Codex,"
