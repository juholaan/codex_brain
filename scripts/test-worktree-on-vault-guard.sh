...
addworktree() { # REPO  SLUG  -> a real linked worktree at REPO/.codex/worktrees/SLUG
  mkdir -p "$1/.claude/worktrees"
  git -C "$1" worktree add -q --detach "$1/.codex/worktrees/$2" HEAD 2>/dev/null
}
...
( cd "$POS/.codex/worktrees/session-abc123" && python3 "$CHECK" --porcelain "$POS" >/dev/null 2>&1 ) \
...
mkdir -p "$NOGIT/.obsidian" "$NOGIT/.codex/worktrees/stray"
