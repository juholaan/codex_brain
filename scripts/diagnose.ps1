...
$claudeMd = Join-Path $Vault "AGENTS.md"
if (Test-Path -LiteralPath $claudeMd) {
  $size = (Get-Item -LiteralPath $claudeMd).Length
  if ($size -lt 200) {
    Warn "AGENTS.md exists but is tiny ($size bytes)" "Re-run /setup-brain Phase 4."
  } else {
...
if (Test-Path -LiteralPath $skillsDir -PathType Container) {
  ...
  if (Test-Path -LiteralPath (Join-Path $skillsDir "ai-brain-starter") -PathType Container) {
    ...
    Warn "ai-brain-starter skill not in ~/.agents/skills/" "Re-run bootstrap to clone it."
  }
  ...
  Bad "~/.agents/skills/ missing" "Codex may not be installed."
}
...
# `rm ~/.codex/hooks/<name>.py` as the supported uninstall -- an absent file is
...
foreach ($f in @($settings, $localSettings)) {
  ...
  if (-not $raw) { continue }
  # Anchor on .codex/hooks/ so the skill's own hooks dir
  # (plugins/codex-brain-starter/hooks/) is not swept in. Separator class
...
if ($wiredNames.Count -gt 0) {
  ...
  if ($missingHooks.Count -eq 0) {
    Ok ("all {0} wired ~/.codex/hooks/ file(s) present on disk" -f $wiredNames.Count)
  } else {
    ...
    $msg = "{0} of {1} wired ~/.codex/hooks/ file(s) MISSING: {2}" -f $missingHooks.Count, $wiredNames.Count, ($missingHooks -join ", ")
    Warn $msg $hint
  }
}
