#!/usr/bin/env python3
"""Single source of truth: vault path -> Codex per-project memory dir.

Codex keys per-project auto-memory under
``~/.codex/projects/<encoded-path>/memory/``. The encoding replaces every
...
def encode_project_key(path: str | os.PathLike) -> str:
    """Encode an absolute filesystem path to Codex's project-dir key.

    Mirrors Codex's own sanitization: abspath, then every non
...
def claude_projects_root() -> Path:
    """``~/.codex/projects`` — overridable via CLAUDE_HOME for tests."""
...
def project_dir_for(vault: str | os.PathLike) -> Path:
    """Return the ``~/.codex/projects/<key>`` dir for a vault.
    ...
    normal case the first time Codex is run from a brand-new vault — it
