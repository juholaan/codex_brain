...
# Codex path-keyed state migration helper (MYC-511 item 4).
#
...
#   - FAIL-LOUD: a zero-match key prints "no Codex session history" and
...
rc=$?
echo "$out" | grep -qi 'no Codex session history' \
...
mkdir -p "$eb3_src/⚙️ Meta" ; echo "brain" > "$eb3_src/AGENTS.md" ; echo "note" > "$eb3_src/note.md"
