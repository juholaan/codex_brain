...
#
# Problem: Codex does not reliably expand ${CLAUDE_PLUGIN_ROOT} when spawning
# hook subprocesses. When the variable is unset, the hook path resolves to
# "/hooks/script.py" (nonexistent), the hook errors, and Codex defaults to
...
if [[ $fixed -eq 0 ]]; then
    ...
    echo ""
    echo "Done. Restart Codex for hook changes to take effect."
fi
