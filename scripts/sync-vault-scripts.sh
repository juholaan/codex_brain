...
#   vaults. scripts/sync-skills.sh only syncs skill->~/.agents/skills; this is
...
#   1. --vault PATH
#   2. $VAULT_ROOT
#   3. parse ~/.codex/settings.json — the installed hooks embed the vault path
...
VAULT_SCRIPTS=(
  ...
  "drift-detection.py"         # rule / AGENTS.md drift detection
...
resolve_vault_from_settings() {
  local settings="$HOME/.codex/settings.json"
  [ -f "$settings" ] || return 1
  [ -n "$PY_CMD" ] || return 1
