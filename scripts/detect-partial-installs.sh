...
check_graphify() {
  ...
  local skill_dir="$HOME/.agents/skills/graphify"
  if [[ ! -d "$skill_dir" ]]; then
...
check_humanizer() {
  local skill_dir="$HOME/.agents/skills/humanizer"
  if [[ ! -d "$skill_dir" ]]; then
...
check_vault_scripts() {
  ...
  [[ -f "$sync" ]] || sync="$HOME/plugins/codex-brain-starter/scripts/sync-vault-scripts.sh"
  if [[ ! -f "$sync" ]]; then
    return
  fi
...
check_ai_brain_starter() {
  local skill_dir="$HOME/plugins/codex-brain-starter"
  if [[ ! -d "$skill_dir" ]]; then
...
check_json_config "$HOME/.codex/settings.json" "claude-settings"
check_json_config "$HOME/.codex/settings.local.json" "claude-settings-local"
check_json_config "$HOME/.codex/.mcp.json" "mcp-config"
...
if [[ "$JSON_OUTPUT" -eq 1 ]]; then
  ...
  if [[ "$AUTO_FIX" -eq 1 ]]; then
    ...
    [[ -f "$sync_script" ]] || sync_script="$HOME/plugins/codex-brain-starter/scripts/sync-vault-scripts.sh"
