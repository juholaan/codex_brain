...
# Called by Codex Stop hook (every turn end).
#
...
# Failures fail-open: never block the user, log to ~/.codex/logs/session-close-errors.log.
...
#
# If $SCRIPT_DIR matches `.../<vault>/.codex/worktrees/<slug>/.../scripts`,
...
ERROR_LOG="$HOME/.codex/logs/session-close-errors.log"
...
if [ -n "$SESSION_ID" ]; then
  MARKER="$HOME/.codex/.closing-signal-${SESSION_ID}.json"
fi
