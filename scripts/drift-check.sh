...
#   stale files is for a human to ask Codex "compare everything" — which is
#   exactly what we're automating away.
#
# What it checks:
#
#   Scope A — installed Codex skills:
#     For every skill bundled under $STARTER/skills/<skill>/, walk every file
#     and compare to ~/.agents/skills/<skill>/<rel-path>. Report any installed
...
#
#   Scope C — vault AGENTS.md rule blocks (only if --vault given):
#     For every templates/rules/*.md, find its top-level heading inside
#     $VAULT/AGENTS.md. If the heading is present but the block underneath
...
#     extracted from AGENTS.md into standalone files (2026-04-12 split).
#
# What it does NOT do:
#   - Modify files. Ever. drift-check is a detector, not a fixer.
#   - Decide what to update. The Codex session-start rule reads this output
...
# ~/plugins/codex-brain-starter (end users) or somewhere else like
# ~/Desktop/ai-brain-starter (maintainer / fork developer).
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
STARTER_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
INSTALL_DIR="$HOME/.agents/skills"
COOLDOWN_FILE="$HOME/.codex/.ai-brain-starter-drift-check-last-run"
IGNORE_FILE="$HOME/.codex/.ai-brain-starter-drift-check-ignore"
TODAY="$(date +%Y-%m-%d)"
FORCE=0
VAULT=""
...
# ── Ignore registry ──────────────────────────────────────────────────────
# Per-user file at ~/.codex/.ai-brain-starter-drift-check-ignore. Each line
...
# "skip permanently" and Codex appends the path here for them.
IGNORE_PATTERNS=""
...
# ── Scope C — vault AGENTS.md rule blocks ─────────────────────────────────
if [[ -n "$VAULT" && -f "$VAULT/AGENTS.md" && -d "$STARTER_DIR/templates/rules" ]]; then
  for rule_file in "$STARTER_DIR/templates/rules"/*.md; do
    ...
    # Check whether that heading appears anywhere in the user's AGENTS.md
    if ! grep -qxF "$heading" "$VAULT/AGENTS.md" 2>/dev/null; then
    ...
    # Heading IS present. Extract the block from AGENTS.md and compare to the
    # template content. Use python for clean block extraction + comparison.
    drift_result=$(python3 - "$VAULT/AGENTS.md" "$rule_file" "$heading" <<'PY'
import sys
...
#      AGENTS.md visual separators between concatenated rule blocks, not
...
PY
    ...
    if [[ "$drift_result" == "DRIFT" ]]; then
      add_drift "vault-rule" "$VAULT/AGENTS.md" "$rule_file" "block heading: $heading"
    fi
  done
fi
...
# copies of templates/rules/*.md (not inlined into AGENTS.md). Compare each
# installed file against its repo counterpart. This scope was added when
# AGENTS.md was split into modular files to stay under the 10K-token read
# limit (2026-04-12).
