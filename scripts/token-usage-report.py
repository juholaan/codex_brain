...
Walks Codex session JSONLs in ~/.codex/projects/<vault-hash>/, extracts
...
Why: Codex does not surface per-session token totals OR per-tool token
...
prepends `~/.codex/projects/`. Override with CLAUDE_SESSIONS_DIR env var.
...
def derive_sessions_dir(vault_root: Path) -> Path:
    """Codex stores session JSONLs under ~/.codex/projects/<sanitized>/.
...
def render_report(
    ...
    out.append(f"*Generated {datetime.now().strftime('%Y-%m-%d %H:%M')} from `~/.codex/projects/.../*.jsonl`. Window: last {days} days. Source: `scripts/token-usage-report.py`.*")
    out.append("")
