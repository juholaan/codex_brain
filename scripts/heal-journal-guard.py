...
WHY THIS EXISTS
  ...
  ~/.codex, not the vault:
    ...
    - its registration in ~/.codex/settings.json PreToolUse under BOTH matchers,
...
WHAT IT VERIFIES + REPAIRS  (idempotent, FAIL-OPEN, fast - never a corpus walk)
  1. REGISTRATION: the guard is wired in ~/.codex/settings.json PreToolUse under every
  ...
  If it repairs anything it appends to ~/.codex/heal-journal-guard.log and surfaces ONE
...
#   POSIX bare      ~/.codex/.../<guard>   or   /Users/d/.claude/.../<guard>
...
def run_session_start() -> None:
    ...
    if _settings_unreadable(settings_path):
        _emit_context(
            "[heal-journal-guard] Cannot read ~/.codex/settings.json (it exists but is "
            ...
            "~/.codex/settings.json.bak-*), then run: python3 "
            "~/plugins/codex-brain-starter/scripts/install-hooks-user-level.py"
        )
        return
...
def self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        posix_t = f"python3 ~/plugins/codex-brain-starter/hooks/{GUARD_BASENAME} 2>/dev/null"
        posix_a = f"/usr/bin/python3 /Users/d/.agents/skills/abs/hooks/{GUARD_BASENAME}"
...
def main() -> int:
    ...
    ap.add_argument("--settings", help="settings.json path (default ~/.codex/settings.json)")
