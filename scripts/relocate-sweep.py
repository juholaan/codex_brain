...
4. A residual REPORT grouped by class + a go/no-go: GO only when zero EXECUTED
   references remain.
5. Characterize the ~/.codex.json blast radius before anyone touches it: dict
...
Options:
  ...
  --claude-json <file>  the big per-host config to characterize (default: ~/.codex.json)
...
SKIP_SUB = ("/.git/", "/.venv/", "/node_modules/", "/.codex/worktrees/",
...
# so this bounds the cost of scanning e.g. a Codex config subtree.
    ...
    if dev_root and os.path.isdir(dev_root):
        for entry in sorted(os.listdir(dev_root)):
            ...
            if os.path.isdir(full):
                roots.append(full)
    # Scope the Codex config dir to where DEPLOYED tooling lives — never the whole
...
def build_report(args):
    ...
    # ~/.codex.json blast radius — characterized separately, always.
    ...
    if text_result is not None:
        ...
        if text is None:
            ...
            if not valid:
                _incomplete("~/.codex.json did not parse — characterize it by hand before editing")
...
def print_human(rep):
    ...
    if cj["present"]:
        ...
        print("\n~/.codex.json blast radius: %d dead dict-key(s) (cosmetic) / %d string-value(s) (load-bearing)  [%s]"
