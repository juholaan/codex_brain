...
machinery_files_in_tree() {  # $1 = vault dir
  ...
  n="$(find "$v" -type f \
        \( -path '*/.git/*' -o -path '*/.smart-env/*' -o -path '*/.codegraph/*' \
           -o -path '*/.codex/worktrees/*' -o -path '*/Sessions/*' \) \
...
churn() {  # $1 = vault dir — simulate a real session's git churn
  ...
  gitq "$v" worktree add "$v/.codex/worktrees/wt1" -b churnbranch HEAD || \
    gitq "$v" worktree add --detach "$v/.codex/worktrees/wt1" HEAD || true
...
VD="$ROOT/D"
make_vault "$VD"
gitq "$VD" worktree add "$VD/.codex/worktrees/live" -b livebranch HEAD
